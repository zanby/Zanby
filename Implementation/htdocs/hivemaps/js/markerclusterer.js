/**
 * Modified version with fix bugs.  Search for @JOE.
 *
 * @name MarkerClusterer (modified)
 * @version 1.01
 * @author Xiaoxi Wu
 * @author Joe Johnston
...
function MarkerClusterer(map, opt_markers, opt_opts) {
  ...
  function addLeftMarkers_() {
    ...
    var leftMarkers = [];
    var len = leftMarkers_.length;
    for (i = 0; i < len; ++i) {
      // @JOE
      //me_.addMarker(leftMarkers_[i], true, null, null, true);
      me_.addMarker(leftMarkers_.shift(), true, null, null, false);
    }
    // @JOE

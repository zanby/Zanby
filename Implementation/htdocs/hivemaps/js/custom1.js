...
jQuery(function($){  
  var dataURL = 'test/data.xml';
  var dataDetailsBaseURL = 'test/details.xml';
  var detailsBaseURL = '/';
  ...
  function loadDetails(id, locTab, infoTab) {
    ...
    $.ajax({url: dataDetailsBaseURL, type: 'GET', dataType: 'xml',
...
for (var is=0;data.length;is++){
  if (data[is].id == id) {
   locTab.contentElem.innerHTML = getLocTabHtml(data[is]);
        infoTab.contentElem.innerHTML = getInfoTabHtml(data[is]);
  }
}

...
//alert (this.settings.theme_href);
...
//alert (tinyMCE.baseURL + '/themes/' + this.settings.theme + '/editor_template' + tinyMCE.srcMode + '.js');
	...
	addMCEControl : function(replace_element, form_element_name, target_document) {
		...
		inst.isHeadline = false;
		

		//MSQ MSQ MSQ 
		var tTheme = 'zanby';
		inst.contentObjectId = form_element_name.substr(8); //alert(inst.contentObjectId);
		if (inst.contentObjectId.substr(inst.contentObjectId.length-2) == '_H')
		{
			var tTheme = 'headline';
			inst.contentObjectId = inst.contentObjectId.substr(0, inst.contentObjectId.length-2);
			inst.isHeadline = true;
			//alert(inst.contentObjectId);
		}
		//MSQ MSQ MSQ
		...
		inst._onAdd(replace_element, form_element_name, target_document, tTheme);
		
		return tinyMCE.idCounter-1;
	},
		...
		{			
			if (!tinyMCE.isIE){
				...
				/*for (var j=0;j<window.frames.length;j++){
					if (window.frames[j].window.document.getElementById('b_'+editor_id))
					{
						var mElement = window.frames[j].window.document.getElementById('b_'+editor_id);//window.frames[j];//.window.document.getElementById('b_'+editor_id);	
					}
				}
				if (mElement){
					var mEnd = mElement.innerHTML.length; 
					mElement.setSelection();//Range(0,mEnd);
					
					//mElement.setSelectionRange();
					mElement.focus();
					//document.body.setSelectionRange();
					//document.body.focus();
				}*/
			} else {
				for (var j=0;j<document.frames.length;j++){
					if (document.frames[j].window.document.getElementById('b_'+editor_id))
					{
						var mElement = document.frames[j].window.document.getElementById('b_'+editor_id);	
					}
				}
				if (mElement){
					mElement.createTextRange().execCommand("SelectAll");
				}
			}
	...
	execCommand : function(command, user_interface, value) {
		...
		switch (command) {
			...
			case "mceAddEditor":
				...
				return tinyMCE.addMCEControl(tinyMCE._getElementById(value), value);
	...
	//alert('111'+tinyMCE.baseURL + "/themes/" + inst.settings.theme + "/css/editor_content.css");
	// Import theme specific content CSS the user specific
	//alert (tinyMCE.baseURL + "/themes/" + inst.settings.theme + "/css/editor_content.css");
		tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/themes/" + inst.settings.theme + "/css/editor_content.css");
		tinyMCE.importCSS(inst.getDoc(), inst.settings.content_css);
		//MSQ
		//tinyMCE.importCSS(inst.getDoc(), inst.settings.advanced_css);
	...
	getButtonHTML : function(id, lang, img, cmd, ui, val) {
		...
		if (tinyMCE.getParam('button_tile_map') && (!tinyMCE.isIE || tinyMCE.isOpera) && (m = this.buttonMap[id]) != null && (tinyMCE.getParam("language") == "en" || img.indexOf('$lang') == -1)) {
			// Tiled button
			x = 0 - (m * 20) == 0 ? '0' : 0 - (m * 20);
			h += '<div style="float:left;"><a id="{$editor_id}_' + id + '" href="javascript:' + cmd + '" onclick="' + cmd + 'return false;" onmousedown="return false;" ' + io + ' class="mceTiledButton mceButtonNormal" target="_self">';
			h += '<img src="'+AppTheme.images+'/decorators/spacer.gif" style="background-position: ' + x + 'px 0" alt="{$'+lang+'}" title="{$' + lang + '}" />';
			h += '</a></div>';
		} else {
			// Normal button
			h += '<div style="float:left;"><a id="{$editor_id}_' + id + '" href="javascript:' + cmd + '" onclick="' + cmd + 'return false;" onmousedown="return false;" ' + io + ' class="mceButtonNormal" target="_self">';
	...
	getMenuButtonHTML : function(id, lang, img, mcmd, cmd, ui, val) {
		...
		return '<div style="float:left;">'+h+'</div>';
	},
...
TinyMCE_Control.prototype = {
	...
	getBody : function() {
		...
		return this.contentBody ? this.contentBody : this.getDoc()? this.getDoc().body : '';
...
//		return this.contentDocument ? this.contentDocument : this.contentWindow.document; // Removed due to IE 5.5 ?
	...
	_onAdd : function(replace_element, form_element_name, target_document, tTheme) {
		var hc, th, tos, editorTemplate, targetDoc, deltaWidth, deltaHeight, html, rng, fragment;
		var dynamicIFrame, tElm, doc, parentElm;
//alert(this.settings.theme);
		// MSQ
		    if (tTheme && this.settings.theme !=='simple') {this.settings.theme = tTheme;} 
		//
		//alert (this.settings.theme);
		...
		{
			//msq
			if (this.settings.theme == 'headline')
			{
				this.settings.default_document = tinyMCE.baseURL + "/blank_h.htm";
			} else {

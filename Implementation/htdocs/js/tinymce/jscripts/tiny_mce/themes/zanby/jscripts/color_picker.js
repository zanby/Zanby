...
function generateWebColorsAK(lang_CPfunc) {
	var el = document.getElementById('webcolors'), h = '', i;
	...
	for (i=0; i<colors.length; i++) {
		h += '<td bgcolor="' + colors[i] + '">'
			+ '<a href="#null" onclick="'+lang_CPfunc+'" onfocus="showColor(\'' + colors[i] +  '\');" onmouseover="showColor(\'' + colors[i] +  '\');">'
			+ '<img border="0" src="images/spacer.gif" width="10" height="10" title="' + colors[i] +  '" alt="' + colors[i] +  '" /></a></td>';
		if ((i+1) % 18 == 0)
			h += '</tr><tr>';
	}
...
function generateNamedColorsAK(lang_CPfunc) {
	var el = document.getElementById('namedcolors'), h = '', n, v, i = 0;
	...
	for (n in named) {
		v = named[n];
		h += '<a href="#null" onclick="'+lang_CPfunc+'" onmouseover="showColor(\'' + n +  '\',\'' + v + '\');" style="background-color: ' + n + '"><!-- IE --></a>'
	}

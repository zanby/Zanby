	...
	if ((strWord != null) && (strWord.length > 0)){
		...
		if (similarityMap.GetCount() > 0){
			...
			for(var index = 0; index < nStringLength; index++){
				if (fLettersOnly && !isctype(strCanonicalizedWord.charAt(index), kSmallLetter)){
					continue;
				}
...
function EvalPwdStrength(oF,sP){
	...
	if(ClientSideBestPassword(sP,gSimilarityMap,gDictionary)){
		GEId(oF).className = "znbPassStrong";
		GEId(oF).innerHTML = 'STRONG';
	}
		...
		GEId(oF).className = "znbPassMiddle";
		GEId(oF).innerHTML = 'MIDDLE';
	}
	else if(ClientSideWeakPassword(sP,gSimilarityMap,gDictionary)){
		GEId(oF).className = "znbPassWeak";
		GEId(oF).innerHTML = 'WEAK';
	}
	else{
		GEId(oF).innerHTML = '';
	}
...
if (type == 3) {
  selectionarray = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz";
}
if (type == 2) {
  selectionarray = "0123456789abcdefghijklmnopqrstuvwxyz";
}
if (type <= 1) {
  selectionarray = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
}
length = selectionarray.length;
...
for (i = 0; i < lngth; i++){
  j = parseInt(Math.random() * length);
  tmpstr = tmpstr + selectionarray.substring(j, j+1);
}	

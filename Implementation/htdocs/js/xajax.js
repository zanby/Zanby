...
function Xajax()
{
    ...
    this.AfterResponseFunctionName = null;
    this.AfterResponseFunctionArgs = null;
	...
	this.loadingFunction = function(){
        if ( !loadingMessage ) {
            ...
            loadingMessage.innerHTML = "<img src='"+AppTheme.images+"/decorators/loading.gif' border=0>";
            loadingMessage.className = "loading";
            document.body.appendChild(loadingMessage);
        }
        loadingMessage.style.top = (document.body.scrollTop + 5) + "px";
        loadingMessage.style.left = (document.body.scrollLeft + 5) + "px";
        loadingMessage.style.display = "";
        checkScrollTimeout = setTimeout("xajax.checkScroll();",1);
	};
	...
	{
	    return this._call(sFunction, xajaxRequestUri,  aArgs, sRequestType);
	}
	this.call_uri = function(sURI, aArgs, sRequestType)
	{
        return this._call("", sURI,  aArgs, sRequestType);
	}
	this._call = function(sFunction, sURI, aArgs, sRequestType)
	{
		var i,r,postData;
		...
		r.onreadystatechange = function()
		{
			...
			if (r.status==200)
			{
				...
				if (r.responseXML && r.responseXML.documentElement) {
					...
					if ( xajax.AfterResponseFunctionName != null ) {
                        $eval = "" + xajax.AfterResponseFunctionName + "(";
                        if ( xajax.AfterResponseFunctionArgs.length > 0 ) {
                            for (i = 0; i < xajax.AfterResponseFunctionArgs.length; ++i) {
                                $eval += "xajax.AfterResponseFunctionArgs["+i+"]";
                                if ( i < xajax.AfterResponseFunctionArgs.length - 1) {
                                    $eval += ", ";
                                }
                            }
                        }
                        $eval += ");";
                        eval($eval);
                        xajax.AfterResponseFunctionName = null;
                        xajax.AfterResponseFunctionArgs = null;
					}
				} else {
	...
	this.addAfterResponseFunction = function(name)
	{
        this.AfterResponseFunctionName = name;
        this.AfterResponseFunctionArgs = [];

	    if ( arguments.length > 1 ) {
            for (i = 1; i < arguments.length; ++i) {
                this.AfterResponseFunctionArgs.push(arguments[i]);
            }
	    }
	}
	//
		...
		for (var i=0; i<xml.childNodes.length; i++)
		{
			...
			if (xml.childNodes[i].nodeName == "cmd")
			{
				...
				for (var j=0; j<xml.childNodes[i].attributes.length; j++)
				{
					...
					else if (xml.childNodes[i].attributes[j].name == "u")
					{
						url = xml.childNodes[i].attributes[j].value;
					}
				}
				...
				try
				{
					...
					{
					    if ( property ) property = YAHOO.ccf.json.parseJSON(property);
					    xajaxRequestManager.showProcessMessage(data, property);
					}
                    else if (cmd=="aa")
                    {
                        if ( property ) property = YAHOO.ccf.json.parseJSON(property);
                        ...
                        MainApplication.showAjaxAlert(property);
                    }
                    else if (cmd=="df")
                    {
                    	var oIF = document.getElementById('xajaxIFDownload'+this.dfIndex);
                    	if ( oIF ) oIF.setAttribute('src','');
                    	else {
                    		oIF = document.createElement('iframe');
                    		oIF.setAttribute('id','xajaxIFDownload'+this.dfIndex);
                    		oIF.setAttribute('src','');
                    		oIF.setAttribute('style','visibility:hidden;width:1px;height:1px;');
                			document.body.appendChild(oIF);                    	
                    	}
                    	oIF.setAttribute('src',url);
                    	this.dfIndex++;
                    }

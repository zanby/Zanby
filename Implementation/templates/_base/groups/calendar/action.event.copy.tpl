...
<div class="prInner prClr3">
    ...
    <col width="65%" />
   	<col width="10%" />
    <thead>
        <tr><th colspan="3" class="prTLeft">{t}Fields marked with asterisk <span class="prMarkRequired">*</span> are required.{/t}
		</th></tr>
	</thead>
    <tbody>
        <tr>
            ...
            <td>
				{form_text name="event_title" value=$objCopyEvent->getTitle()|escape:html}
			</td>
			<td class="prTip"></td>
        </tr>
        <tr>
			<td class="prTRight">
				<label for="event_description">{t}Notes:{/t}</label>
			</td>
			<td>
				{form_textarea name="event_description" id="eventDescription" value=$objCopyEvent->getDescription()|escape:html rows="5"}
			</td>
			<td class="prTip"></td>
        </tr>
        <tr>
			<td class="prTRight">
				<label for="event_tags">{t}Tags:{/t}</label>
			</td>
            <td>
                {form_text name="event_tags" value=$formParams.event_tags|escape:html}
			</td>
			<td class="prTip"></td>
		</tr>
        <tr>
            <td class="prTRight">
				<label><span class="prMarkRequired">*</span>{t}Date:{/t}</label>
			</td>
            <td>
				...
				<div class="prFloatLeft"><a href="#null" id="dtstartCalendarContainerLink"><img src="{$AppTheme->images}/decorators/event/icon-calendar.gif" /></a></div>
				<div id="dtstartCalendarContainerDIV"></div>
            </td>
			<td class="prTip"></td>
        </tr>
        <tr>
            <td class="prTRight"><span class="prMarkRequired">*</span> <label for="event_time_hour"> {t}Event Time:{/t}</label>
			</td>
            <td>
		...
		<tr>
			<td class="prTRight">
				<label>{t}Event Time Zone:{/t}</label>
			</td>
            <td>
                ...
                <div id="event_timezone_DIV" class="prInnerLeft prInnerRight prFloatLeft"{if $formParams.event_timezone_mode != 1} style="display:none;"{/if}>{form_select name="event_timezone" id="event_timezone" selected=$formParams.event_timezone style="width:285px;" options=$timezones}</div>
            </td>
			<td class="prTip"></td>
		</tr>
        <tr>
            ...
            <td id="pictureSelectContent">
				<div id="EventPictureBlock"{if !$objCopyEvent->getPictureId()} style="display:none"{/if}>
					{if $objCopyEvent->getPictureId()}
						<img src="{$objCopyEvent->getEventPicture()->setWidth(75)->setHeight(75)->getImage($user)}" style="margin:0px;" id="EventImageObj"/>
					{else}
						<img src="{$AppTheme->images}/decorators/event/no_event_image.jpg" style="margin:0px;" id="EventImageObj" />
					{/if}
					<div class="prFloatLeft prInnerLeft">
						...
						<div class="prIndentTopSmall">
							<a href="#null" onclick="xajax_doAttachPhotoDelete();return false;">{t}Delete Picture{/t}</a>
						</div>
					</div>
				</div>
				<div id="EventPictureBlockNONE"{if $objCopyEvent->getPictureId()} style="display:none"{/if} class="prClr2">
					<img src="{$AppTheme->images}/decorators/event/no_event_image.jpg" class="prFloatLeft" />
					<div class="prFloatLeft prInnerLeft">
					<a href="#null" onclick="xajax_doAttachPhoto(YAHOO.util.Dom.get('event_picture_id').value);return false;">+ {t}Attach Picture{/t}</a>
					</div>
				</div>
...
<div class="prInnerSmallTop prTRight">
	...
	{t}or&nbsp;{/t}<a href="{$CurrentGroup->getGroupPath('calendar.list.view')}">{t}Cancel{/t}</a>
</div>
{/form}

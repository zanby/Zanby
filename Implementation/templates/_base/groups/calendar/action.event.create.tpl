...
<col width="10%" />
<thead>
    <tr><th colspan="3" class="prText5 prTLeft">{t}Fields marked with asterisk <span class="prMarkRequired">*</span> are required.{/t}</th></tr>
</thead>
<tbody>
    <tr>
        <td class="prTRight"><span class="prMarkRequired">*</span> <label for="event_title"> {t}Event Title:{/t}</label></td>
        <td>{form_text name="event_title" value=$objEvent->getTitle()|escape:html}</td>
        <td class="prTip"></td>
    </tr>
    <tr>
        <td class="prTRight"><label for="event_description">{t}Notes:{/t}</label></td>
        <td>{form_textarea id="eventDescription" name="event_description" value=$objEvent->getDescription()|escape:html rows="5"}</td>
        <td class="prTip"></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <div class="prFloatLeft"><a href="javascript:void(0)" id="dtstartCalendarContainerLink"><img src="{$AppTheme->images}/decorators/event/icon-calendar.gif" alt="" /></a></div>
    ...
    <tr>
        <td class="prTRight"><span class="prMarkRequired">*</span> <label for=""> {t}Event Time:{/t}</label></td>
        <td>
            <label for="" class="prEventTimeLabel">{t}Starts at{/t} </label>
            ...
            <div class="prIndentTop prClr3">
                <label for="" class="prEventTimeLabel">{t}Duration{/t}</label>
            ...
            <div class="prIndentTop">{form_checkbox name="event_is_allday" id="event_is_allday" checked=$formParams.event_is_allday value="1" onclick="CreateEventApp.onAllDayClick();"}<label for="event_is_allday" class=""> {t}This is an all day event{/t}</label></div>
        </td>
        <td class="prTip"></td>
    </tr>
    <tr>
    ...
    <tr>
        <td class="prTRight"><span class="prMarkRequired">*</span> <label for="event_event_type_1"> {t}Event Type:{/t}</label></td>
        <td>
    ...
    <tr>
        ...
        <td class="prClr2">
            <div id="EventPictureBlock"{if !$objEvent->getPictureId()} style="display:none"{/if}>
                {if $objEvent->getPictureId()}
                    <img alt="" src="{$objEvent->getEventPicture()->setWidth(75)->setHeight(75)->getImage($user)}" class="prFloatLeft" id="EventImageObj"/>
                {else}
                    <img alt="" src="{$AppTheme->images}/decorators/event/no_event_image.jpg" class="prFloatLeft" id="EventImageObj" class="prFloatLeft" />
                {/if}
                <div class="prFloatLeft prIndentLeftSmall">
                    ...
                    <div class="prIndentTopSmall"><a href="javascript:void(0)" onclick="xajax_doAttachPhotoDelete();return false;">{t}Delete Picture{/t}</a></div>
                </div>
            </div>
            <div id="EventPictureBlockNONE"{if $objEvent->getPictureId()} style="display:none"{/if}>
                <img alt="" src="{$AppTheme->images}/decorators/event/no_event_image.jpg" class="prFloatLeft" />
                <div class="prFloatLeft prIndentLeftSmall">
                    <a href="javascript:void(0)" onclick="xajax_doAttachPhoto(YAHOO.util.Dom.get('event_picture_id').value);return false;"}>+  {t}Attach Picture{/t}</a>
                </div>
            </div>
            {form_hidden name="event_picture_id" id="event_picture_id" value=$objEvent->getPictureId()|default:0}
        </td>
        <td class="prTip"></td>
    </tr>
</tbody>
			...
			<table class="prForm" id="invite-guests-options-full">
				...
				<tbody>
					...
					<tr>
					   ...
					   <td>
						   {form_checkbox name="event_invite_entire_group" id="event_invite_entire_group" value="1" checked=$formParams.event_invite_entire_group onclick="xajax_doInviteEntireGroup('"|cat:$CurrentGroup->getId()|cat:"', this.checked, CreateEventApp.getInviteGroupObjects() );"}<label for="event_invite_entire_group"> {if $CurrentGroup->getGroupType() == 'simple'}{t}Invite entire Group{/t}{else if $CurrentGroup->getGroupType() == 'family'}{t}Invite entire Group Family{/t}{/if}</label>

						   <div class="prInnerTop">{t}{tparam value=$SITE_NAME_AS_STRING}Enter an email address or a %s username, separated by a comma.{/t}</div>
						   ...
						   <div class="prInnerTop">
							   ...
							   <div class="prInnerSmallTop"><a href="javascript:void(0)" onclick="xajax_addFromAddressbook(); return false;">{t}Insert addresses from Address Book{/t}</a></div>
                           </div>
                           {if FACEBOOK_USED}
                            ...
                            {/if}
						   <div class="prInnerSmallTop" style="{if !$formParams.event_invitations_lists}display:none{/if}" id="EventInviteListsObjects">
						   {include file="groups/calendar/action.event.template.contact.list.tpl"}
						   </div>
						   <div class="prInnerSmallTop" style="{if !$formParams.event_invitations_groups}display:none{/if}" id="EventInviteGroupsObjects">
						   {include file="groups/calendar/action.event.template.contact.group.tpl"}
						   </div>
						   <div class="prInnerTop">{form_textarea id="inv_emails" name="event_invitations_emails" value=$formParams.event_invitations_emails|escape:html}</div>
					...
					<tr>
					   ...
					   <td class="prInnerTop">
							...
							{form_checkbox name="event_allow_guests_invitation" id="event_allow_guests_invitation" value="1" checked=$formParams.event_allow_guests_invitation}
							<span><label for="event_allow_guests_invitation">{t}Allow guests to invite other people{/t}</label></span><br />
							{form_checkbox name="event_display_guests" id="event_display_guests" value="1" checked=$formParams.event_display_guests}
							<span><label for="event_display_guests">{t}Display Guest List to Invited Guests{/t}</label></span>

...
<div class="prClr3 prInner">
	<div>
        <label for="search_venue_category">{t}Sort by type:{/t} </label> 
	...
	{if $countVenues>0}
    <div>
    	{$aSearches.wp*10-9}-{if $countVenues > $aSearches.wp*10}{$aSearches.wp*10}{else}{$countVenues}{/if} {t}{tparam value=$countVenues}of %s{/t}
        ...
        {if $aSearches.wp*10 < $countVenues}<a href="#null" onclick="setSearches('p','{$aSearches.p+1}'); xajax_loadSavedVenues(getSearches()); return false;">{t}Next{/t} &raquo;</a>{/if}
    </div>
	{/if}
    <table cellspacing="0" cellpadding="0" class="prResult">
        ...
        {foreach from=$aoWorldwideVenuesList item = venue}
        	<tr>
                ...
                <td class="">
                	<span>{$venue->getDescription()|escape:html}</span>
                </td>
                <td class="">
                    ...
                    <a href="#null" onclick="xajax_copyVenue({$venue->getId()},getMouseCoordinateX(event), getMouseCoordinateY(event)); return false;" title="Add"><img src="{$AppTheme->images}/buttons/add.gif" /></a>
                    <a href="#null" title="Delete" onclick="xajax_deleteVenue({$venue->getId()},getMouseCoordinateX(event), getMouseCoordinateY(event)); return false;"><img src="{$AppTheme->images}/buttons/close.gif" /></a>
                </td>
            </tr>
        {/foreach}
        </tbody>

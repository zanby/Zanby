    ...
    <div>
        ...
        <div class="prPopupHeight">
            <div class="prInnerTop prClr2">{$infoPaging} {$linkPaging}</div>
                        ...
                        <tr id="ajax_contact_row_{$cid}" {if ($smarty.foreach.event_adrb.iteration % 2) == 0} class="prEvenBg"{else} class="prOddBg"{/if}>
                            ...
                            <td>
                                <a href="{$contact->profile}">
                                    {if $contact->avatar}
                                        <img alt="" src="{$contact->avatar->setWidth(37)->setHeight(37)->setBorder(1)->getImage()}" border="0" />
                                    {/if}
                                </a>
                            </td>
                        </tr>
                ...
                <div class="prTRight prInnerTop">{$infoPaging} {$linkPaging}</div>
            </div>
        </div>
        <div class="prInnerTop prTCenter prClr">
           {t var="in_submit"}Add contacts to the list{/t}
           {form_submit name="add" value=$in_submit}
        </div>
        {/form}
    </div>
{else}

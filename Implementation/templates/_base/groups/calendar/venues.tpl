<!-- form container -->
                ...
                {form_radio id="vt_1" name="event_venue_type" value='no' checked=$formParams.venue_type onclick="changevenueto('no')"}<label for="vt_1" class=""> {t}This event has no set  venue.{/t}</label>
				</div>
				<div class="prIndentTopSmall"> 
                {form_radio id="vt_2" name="event_venue_type" value='worldwide' checked=$formParams.venue_type onclick="xajax_chooseSavedWWVenue(); return true;"}<label for="vt_2" class=""> {t}This event is worldwide.{/t}</label>
				</div>
				<div class="prIndentTopSmall"> 
                {form_radio id="vt_3" name="event_venue_type" value='simple' checked=$formParams.venue_type onclick="xajax_chooseSavedVenue(); return true;"}<label for="vt_3" class=""> {t}This event takes place at the following venue.{/t}</label>
				</div>
...
<div id="add_show">
    ...
    <div id="worldwide_venue_block" class="prSubNavBlock" {if $formParams.venue_type != 'worldwide'} style="display:none" {/if}>
		{include file="groups/calendar/ww.venues.tabs.tpl" active='add'}
		<div class="prSubNavContBlock" id="worldwide_venue_body">
            {if $formParams.venueId && $formParams.venue_type == "worldwide"}
                {include file="groups/calendar/ww.venues.view.tpl"}
            {else}
                {include file= "groups/calendar/ww.venues.index.tpl"}
            {/if}
        </div>
    </div>
    <!-- /worldwide venue -->
    ...
    <div id="simple_venue_block" class="prSubNavBlock" {if $formParams.venue_type != 'simple'} style="display:none" {/if}>
		{include file="groups/calendar/venues.tabs.tpl"  active='add'}
			<div class="prSubNavContBlock" id="simple_venue_body">
            {if $formParams.venueId && $formParams.venue_type == "simple"}
                {include file="groups/calendar/venues.view.tpl"}
            {else}
                {include file= "groups/calendar/venues.index.tpl"}
            {/if}
        </div>
    </div>
    <!-- /simple venue block -->
    ...
    <div id="saved_worldwide_venue_block" class="prSubNavBlock" style="display:none"> 
		{include file="groups/calendar/ww.venues.tabs.tpl"  active='saved'}
       
        <div class="prSubNavContBlock" id="saved_worldwide_venue_body">
            {include file="groups/calendar/ww.venues.saved.index.tpl"}
        </div>
    ...
    <div id="saved_simple_venue_block" class="prSubNavBlock" style="display:none">       
            {include file="groups/calendar/venues.tabs.tpl"  active='saved'}
       
        <div class="prSubNavContBlock" id="saved_simple_venue_body">
            {include file="groups/calendar/venues.saved.index.tpl"}
        </div>
    ...
    <div id="find_simple_venue_block" class="prSubNavBlock" style="display:none">       
            {include file="groups/calendar/venues.tabs.tpl"  active='find'}        
        <div class="prSubNavContBlock" id="find_simple_venue_body">
            {include file="groups/calendar/venues.search.index.tpl"}
        </div>
    </div>
    <!-- /find simple -->    
</div>
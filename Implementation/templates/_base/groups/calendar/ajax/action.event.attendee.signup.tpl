...
{literal}
    <script type="text/javascript">
        ...
        $(function(){ FB.XFBML.Host.parseDomElement(document.getElementById('fbConnectButtonPlaceholder')); })
    </script>
{/literal}
{/if}
<div>
    ...
    <div class="prInnerTop prClr3 prBorderBottom prInnerBottom" id="fbConnectButtonPlaceholder">
        <div class="prFloatLeft">{t}{tparam value=$BASE_URL}{tparam value=$LOCALE}To RSVP you need to <a id="btnSignIn" href="%s/%s/users/login/">Sign In</a>{/t} {if $FACEBOOK_USED} {t}Or{/t} </div><div class="prIndentLeftSmall prFloatLeft"><fb:login-button onlogin="FBApplication.onrsvplogin_ready('{$event_id}', {$uid}, '{$view}');"  size="medium" length="long"></fb:login-button></div>{else}</div>{/if}
    </div>

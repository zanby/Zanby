...
<div class="prEvents-calendar">
	<div class="prClr3">
			...
			{if $objPrevDate}<a  href="{$CurrentGroup->getGroupPath()}calendar.month.view/year/{$objPrevDate->toString('yyyy')}/month/{$objPrevDate->toString('MM')}/">&laquo;</a>{/if}
			&nbsp;{$objCurrDate->toString('MMMM')}, {$objCurrDate->toString('yyyy')}&nbsp;
			{if $objNextDate}<a href="{$CurrentGroup->getGroupPath()}calendar.month.view/year/{$objNextDate->toString('yyyy')}/month/{$objNextDate->toString('MM')}/">&raquo;</a>{/if}
		</h2>
		<div class="prClr3">
			{form from=$form_sel}
				{form_hidden name="url" value=$CurrentGroup->getGroupPath('calendar.month.view')}
			...
			<tr>
				{foreach from=$week->getDays() item=day}
					...
					{if $day->getMonth() != $month->getMonth()}
						{if $arrDates[$strDate]}{assign var='tdStyle' value='prPreviewMonth prPreviewMonth-active'}{*прошлый месяц с событиями*}
					...
					<td class="{$tdStyle}">
						<div class="prTRight">
										...
										{if $currIndex <= 5}
											...
											<span class="prEllipsis prCalendarCell">
												...
												*}
												...
												{else}
													{if $user->getId() && null !== $userAttendee && $userAttendee->getAnswer() == 'NONE'}
														<a class="ellipsis_init" style="color:#7f7f7f;" href="{$objCEvent->getOwner()->getGroupPath('calendar.event.view')}id/{$currentEvent.id}/uid/{$currentEvent.uid}/year/{$currentEvent.year}/month/{$currentEvent.month}/day/{$currentEvent.day}/"  onmouseover="MonthViewApp.showEventTooltip('{$currentEvent.id}{$currentEvent.uid}{$currentEvent.year}{$currentEvent.month}{$currentEvent.day}', this);" onmouseout="MonthViewApp.hideEventTooltip('{$currentEvent.id}{$currentEvent.uid}{$currentEvent.year}{$currentEvent.month}{$currentEvent.day}', this);">{*if $currentTimeOfDay != 'allday'}{$currentTimeOfDay}<br>{/if*}{$currentEvent.title|escape:html}</a> <span style="color:#7f7f7f;">(</span><img src="{$AppTheme->images}/decorators/horly_glass.gif" alt="" /><span style="color:#7f7f7f;">)</span>
													{else}
											...
											{stringbuilder mode='append' var='dContent'}
												<div  id="TooltipLinkHiddenContent{$currentEvent.id}{$currentEvent.uid}{$currentEvent.year}{$currentEvent.month}{$currentEvent.day}" style="display:none">
													...
													<div>
														{if $objCEvent->getOwnerType() == 'user'}
															{if $user->getId() && null !== $userAttendee && $userAttendee->getAnswer() == 'NONE'}
																<a style="color:#7f7f7f;" href="{$objCEvent->getOwner()->getUserPath('calendar.event.view')}id/{$currentEvent.id}/uid/{$currentEvent.uid}/year/{$currentEvent.year}/month/{$currentEvent.month}/day/{$currentEvent.day}/"><b>{$objCEvent->getTitle()|escape:html}</b></a> <span style="color:#7f7f7f;"><b>(</b></span><img src="{$AppTheme->images}/decorators/horly_glass.gif" alt="" /><span style="color:#7f7f7f;"><b>)</b></span>
															{else}
														...
														{else}
															{if $user->getId() && null !== $userAttendee && $userAttendee->getAnswer() == 'NONE'}
																<a style="color:#7f7f7f;" href="{$objCEvent->getOwner()->getGroupPath('calendar.event.view')}id/{$currentEvent.id}/uid/{$currentEvent.uid}/year/{$currentEvent.year}/month/{$currentEvent.month}/day/{$currentEvent.day}/"><b>{$objCEvent->getTitle()|escape:html}</b></a> <span style="color:#7f7f7f;"><b>(</b></span><img src="{$AppTheme->images}/decorators/horly_glass.gif" alt="" /><span style="color:#7f7f7f;"><b>)</b></span>
															{else}
													...
													<div>
														{if $objCEvent->getOwnerType() == 'group'}
																{t}Group event :{/t}
																<a href="{$objCEvent->getOwner()->getGroupPath('summary')}">{$objCEvent->getOwner()->getName()|escape:html}</a><br />
														{/if}
													</div>
													<div>
													...
													<div class="prFloatRight prClr3">
														{if $objCEvent->getOwnerType() == 'user'}
															<a href="{$objCEvent->getOwner()->getUserPath('calendar.event.view')}id/{$objCEvent->getId()}/uid/{$objCEvent->getUid()}/year/{$objCEvent->getDtstart()->toString('yyyy')}/month/{$objCEvent->getDtstart()->toString('MM')}/day/{$objCEvent->getDtstart()->toString('dd')}/" style="text-decoration:none;">{t}See details{/t} &raquo;</a>
														{else}
															<a href="{$objCEvent->getOwner()->getGroupPath('calendar.event.view')}id/{$objCEvent->getId()}/uid/{$objCEvent->getUid()}/year/{$objCEvent->getDtstart()->toString('yyyy')}/month/{$objCEvent->getDtstart()->toString('MM')}/day/{$objCEvent->getDtstart()->toString('dd')}/" style="text-decoration:none;">{t}See details{/t} &raquo;</a>
														{/if}
													</div>
											{/stringbuilder}
										{/if}
							...
							{/if}
							{if $currIndex > 5}
							<li class="prCalendar-moreevents"><a href="#" onClick="xajax_doViewDayDetails('{$strDate}'); return false;" class="">+{t}{tparam value=$currIndex-5}%s more{/t}</a></li>
							{/if}
							{if $canCreateEvent}
...
<div id="TopicTooltipContent" class="TooltipContent" style=" z-index:10000; position:absolute; display:none; width: 400px; padding: 5px; font-size:12px" onmouseover="onTooltipOver();" onmouseout="onTooltipOut();"></div>

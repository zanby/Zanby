...
{form from=$form id="uploadVideosForm" enctype="multipart/form-data"}
	...
	<tr>
			...
			{form_hidden id="gallery_title" name="gallery_title" value=$gallery->getTitle()|escape:"html"} </td>
			<td></td>
		</tr>
	<tr>
			<td></td>
			<td> {form_radio id="uploadversionSwitcher" name="versionSwitcher"  value="0" checked=$versionSwitcher onclick="turnOnUploadVersion();"}
			<label for="uploadversionSwitcher">{t}Upload Video{/t}</label>
			<div class="prIndentTopSmall"> {form_radio id="URLversionSwitcher" name="versionSwitcher"  value="1" checked=$versionSwitcher onclick="if (swfversion == true) upload1.cancelUpload();turnOnURLVersion();"}
	...
	<div id="fields_table" _style="display:none;" class="prInnerTop prInnerRight prInnerLeft prClr3">
			<table class="prForm">
			<col width="25%" />
			<col width="49%" />
			<col width="25%" />
			...
			<tr>
					<td></td>
					<td><!--div class="prClr2">
							<div class="prFloatLeft prIndentTopSmall"> {linkbutton name="Upload Video" onclick="beforeupload(); return false;"} </div>
							<div class="prFloatLeft prIndentLeftSmall">{t}or{/t}&nbsp;<a id="btnCancel1" href="#" onclick="cancelupload(function (){literal}{beforecancel();}{/literal}); return false;">{t}Cancel{/t}</a> {t}and go back to videos.{/t} </div>
						</div--></td>
					<td></td>
				</tr>
		</table>
	...
	<div class="prInnerTop prInnerRight prInnerLeft" id="SWFUpload" style="display:none;">
			...
			<tr>
					...
					<td><div id="flashUI1" class="prClr">
							...
							<div class="prClr3">
							<div class="prFloatLeft prIndentBottomSmall">{t var="in_button"}Choose Files{/t}{linkbutton id = "browse" name=$in_button onclick="upload1.selectFiles(); return false;"}</div>
	...
	<div id="URLVersion" style="display:none;" class="prInner prClr3">
	<table class="prForm">
			<col width="25%" />
			<col width="49%" />
			<col width="25%" />
...
<div class="prTCenter prIndentTop prIndentRight">
 {t var="in_button_2"}Upload Video{/t}{linkbutton name=$in_button_2 onclick="beforeupload(); return false;"}
					<span class="prIEVerticalAling prIndentLeftSmall">{t}or{/t}&nbsp;<a id="btnCancel1" href="#" onclick="cancelupload(function (){literal}{beforecancel();}{/literal}); return false;">{t}Cancel{/t}</a> {t}and go back to videos.{/t} </span>
				</div>
	{/form} 
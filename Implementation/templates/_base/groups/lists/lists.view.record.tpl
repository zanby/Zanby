<div class="prIndentTopLarge prClr3">
	...
	<div class="prListDescrRight">  
		<div>
			<a href="#" onclick="lock_content(); xajax_list_view_expand('{$record->getId()}', null); return false;" class="prText2">{$record->getTitle()|wordwrap:30:"\n":true|escape}</a> {if $record->getExtraTitleStr()}<span>{$record->getExtraTitleStr()}</span>{/if}
		...
		{if $record->getEntry()}
			<p class="prInnerSmallTop">
				...
				<a href="#" onclick="lock_content(); xajax_list_view_expand('{$record->getId()}', null); return false;">{t}Read More{/t}</a>
			</p>            
		{/if}
		<p class="prInnerSmallTop">
			<a href="#" onclick="lock_content(); xajax_list_view_expand({$record->getId()}, null); return false;">
			{if $record->getCommentsCount()!=1}

	...
	<table class="prForm">
		...
		<tr>
			...
			<td class="prTRight">
				...
				{linkbutton name=$in_button_2 onclick="xajax_list_view_save("|cat:$record->getId()|cat:" ,xajax.getFormValues('record_form_"|cat:$record->getId()|cat:"')); return false;"}
			</td>
			<td></td>
		</tr>
		</tbody>
...
{elseif $Warecorp_List_AccessManager->canAddListItem($user)}
    <h3>{t}+ Add list item{/t}</h3>
    <div id="record_errors_{$record->getId()}">
        {include file="groups/lists/errors.tpl"}
    </div>
    ...
    <table class="prForm">
        ...
        {if HTTP_CONTEXT == 'zccf' || HTTP_CONTEXT == 'zccf-alt' || HTTP_CONTEXT == 'zccf-base'}
        <tr {if !$showExtraFields} style="display:none;"{/if}>
            <td class="prTRight"><label for="item_tags">&nbsp;</td>
			<td colspan=2><div class="prTip">{t}Tags are a way to group your lists and are separated by spaces. Use quotes around multi-word tags like "Public LIbrary"{/t}</div></td>
        </tr>
        {/if}
        <tr>
            <td></td>
            <td>
				{t var="in_button_3"}+ Add Item{/t}
                {linkbutton name=$in_button_3 onclick="lock_content(); xajax_list_view_save('new',xajax.getFormValues('record_form_new'));  return false;"}
            </td>
			<td></td>
        </tr>
    </tbody>

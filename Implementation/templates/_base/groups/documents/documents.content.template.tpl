{if !$foldersList && !$documentsList}
	<div class="prTCenter">{t}No Documents{/t}</div>
{else}    
		...
		<tr{if $smarty.foreach.docs.iteration % 2} class="prZebraBg2"{else} class="prZebraBg1"{/if}>
			<td class="">
				...
				<input type="checkbox" class="item-checkbox prNoBorder" name="files_ch[]" value="{$d->getId()}" 
					...
					{if $d->canBeShared($CurrentGroup, $currGroup, $user->getId())} canshare='1'{else} canshare='0'{/if}
					{if $d->canBeUnShared($CurrentGroup, $user->getId())} canunshare='1'{else} canunshare='0'{/if}
			...
			<td>
				...
				<div {if !$d->getShare()}class='drag-source'{/if} document="{$d->getId()}">
					...
					{if $d->getIsLink()}
						{if $d->getShare()}{t}(Shared){/t}&nbsp;{/if}<a href="{$d->getOriginalName()|escape}" target="_blank" title="{$d->getOriginalName()|escape} {$d->getDescription()} | {$d->getFileSize()}">
						<span>{if $d->getShare()}{$d->getOriginalName()|escape}{else}{$d->getOriginalName()|escape}{/if}</span></a>
						| <span>{$d->getFileSize()}</span>
					{else}
						{if $d->getShare()}{t}(Shared){/t}&nbsp;{/if}<a href="{$CurrentGroup->getGroupPath('docget')}docid/{$d->getId()}/"  id="docid{$d->getId()}" title="{$d->getOriginalName()|escape} {$d->getDescription()} | {$d->getFileSize()}">
						<span>{if $d->getShare()}{$d->getOriginalName()|escape}{else}{$d->getOriginalName()|escape}{/if}</span></a>
						| <span>{$d->getFileSize()}</span>
					{/if}
					</div>
			...
			<td class="prText5">
				{t}{tparam value=$d->getUpdateDate()|user_date_format:$user->getTimezone()|date_locale:'DATETIME_SHORT'} %s{/t}
			</td>
		</tr>
		{/foreach}
	</table>

...
{if !$list_display_type}
	...
	{foreach from=$listsCategories item=item key=key}
		 {if $displayCategories[$key] && $listsList->getListsListByTypeSorted($key, $list_default_index_sort)}
		...
		{foreach from=$currentList->getRecordsList($aSort[$list_default_sort]) item=item key=key name=cl}
	        {if $smarty.foreach.cl.iteration <= $list_display_number_in_each_category}
				<li class="prInnerTop">
					...
					<p><span class="prText4">{t}by{/t} {$item->getCreator()->getLogin()}</span></p>
					 {if $list_show_summaries && $item->getEntry()}
						<p class="prInnerSmall">{$item->getEntry()|escape:'html'}  
						&nbsp;&nbsp;<a href="{$currentList->getListPath()}">{t}More{/t}&nbsp;&raquo;</a></p>
					 {/if}
					 {if $currentList->getRanking()}
						<div class="prInnerSmall prClr">
							{*<ul class="prCO-Ranking prFloatLeft">
	                            <li style="width:{math equation="x * y" x=$item->getRank() y=15 format="%.2f"}px;">&nbsp;</li>
			                      </ul>*}
							<a href="{$currentList->getListPath()}" class="prFloatLeft">{$item->getCommentsCount()}&nbsp;{t}comments{/t}</a>
						</div>
					 {/if}
				</li>
			{/if}	
...
<div class="prInnerTop">
  <a class="prLink2" href="{$currentGroup->getGroupPath('lists')}">{t}Browse all lists{/t} &raquo;</a>
</div>

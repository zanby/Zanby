		...
		<div class="prCOSectionHeader">
			...
			<ul class="prInnerBottom">
				{foreach from=$groupsList item=current name=grList}
					...
					<li class="prClr3{if !$smarty.foreach.grList.first} prIndentTop{/if}" id="gpdiv_{$iter}_{$cloneId}" style="display: {if $currUnHide || ($auto_disp_simple && !in_array($currId,$not_new_groups))}block{else}none{/if}">
						<span class="prFloatLeft"><a href="{$current->getGroupPath('summary')}">{$current->getName()|escape:'html'}</a></span>
					</li>
				{/foreach}
			</ul>

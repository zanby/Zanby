			...
			{if ($event->getId() == $currentG && !$uje) || $event_display_style == 1}
				...
				<div>
					<div>
						...
						<div>
							<h4><a href="{$event->entityURL()}">{$event->getTitle()|escape:"html"}</a></h4>
...
{elseif $event_display_style == 3}
	{if $events_show_calendar}
		{*<h3 class="prTCenter prInnerTop">
			<a  href="{$CurrentGroup->getGroupPath()}calendar.month.view/year/{$objPrevDate->toString('yyyy')}/month/{$objPrevDate->toString('MM')}/">&laquo;</a>
			&nbsp;{$objCurrDate->toString('MMMM')}, {$objCurrDate->toString('yyyy')}&nbsp;
			<a href="{$CurrentGroup->getGroupPath()}calendar.month.view/year/{$objNextDate->toString('yyyy')}/month/{$objNextDate->toString('MM')}/">&raquo;</a>
		</h3>*}
		<!-- ======================================== OLD ======================================= -->
					...
					{foreach from=$_datehash item=_id_id key=_time}
						{foreach from=$_id_id item=_info}
							...
							<div>
								...
								<div>
									<h4>sssss<a id="ddGroupEvents_{$cloneId}_{$event->getId()}_{$_dateval|replace:'-':'_'}" href="{$event->entityURL()}" >{$event->getTitle()|escape:"html"}</a></h4>

...
<table cellspacing="0" cellpadding="0" width="100%">
    ...
    <col width="34%" />
    <tbody>
        <tr> {foreach name=documents from=$documents_ids item=id key=key}
            {assign var="myIteration" value=$smarty.foreach.documents.iteration-1}
            <td>
                <!-- Content object doc item -->
                <div class="prTCenter" id="document_{$cloneId}_{$key}"> {if $id}
                    <!-- -->
                    <div>
                        <a {if !$disable_click} href="#null" onclick="xajax_select_document('{$id}', '{$key}', getMouseCoordinateX(event), getMouseCoordinateY(event), 'document_{$cloneId}_{$key}', '{$cloneId}');return false;"{else}href="{$currentUser->getUserPath("docget/docid")|cat:$documents_objects[$key]->getId()}"{/if}><img src="{$documents_objects[$key]->getImageFileNameByExtension($documents_objects[$key]->getFileExt(), 'big')}" align="top" /></a>
                        {if !$disable_click}
                        ...
                        {else}<a href="#null" onclick="removeSingleDDDocumentSlot('{$cloneId}', {$myIteration});return false;"><img src="{$AppTheme->images}/decorators/profile-marker.gif" alt="" title="" /></a>{/if}
                    ...
                    {if !$disable_click}
                    <div>
                        <a href="#null" onclick="xajax_select_document('{$id}', '{$key}', getMouseCoordinateX(event), getMouseCoordinateY(event), 'document_{$cloneId}_{$key}', '{$cloneId}');return false;"><img src="{$AppTheme->images}/decorators/icoUnknown.gif" align="top" /></a>
                        {if !($smarty.foreach.documents.first && $smarty.foreach.documents.last)}

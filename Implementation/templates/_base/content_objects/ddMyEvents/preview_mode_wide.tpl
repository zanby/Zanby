<div class="themeA"> {include file="content_objects/headline_block_view.tpl"}
    ...
    <div class="prInner prClr3{if !$smarty.foreach.threads.first}{/if}">
        <div class="prFloatLeft">
            ...
            <div class="prIndentTop">
                    ...
                    {if $event->getOwner()->EntityTypeName == 'user'} 
                        <img src="{$event->getOwner()->getAvatar()->setWidth(28)->setHeight(28)->getImage()}"  alt="" title=""  class="prVMiddle" /><br />
                {$event->getOwner()->getLogin()|escape:html}
                    {else} 
                        <img src="{$event->getCreator()->getAvatar()->setWidth(28)->setHeight(28)->getImage()}"  alt="" title=""  class="prVMiddle" /><br />
        ...
        <div class="prFloatRight">
            ...
            <div class="prInnerTop">
                ...
                <ul>
                    {foreach from=$event->getLists()->setFetchMode('object')->getList() item=item}
                        <li><a href="{$item->getListPath()}">{$item->getTitle()|escape:'html'}</a></li>
                    {/foreach}
                </ul>
            </div>
            {/if}
            <div class="prInnerTop">
                    <span>{t}NOTE:{/t}</span> 
                    {if $event->getDescription()}{$event->getDescription()}
                    {else}none{/if} 
                </div>
                <div class="prInnerTop"> 
                ...
                <a href="#null" onclick="xajax_doAttendeeEvent('{$event->getId()}', '{$event->getUid()}', 'month', 0, '{$objEventDtstart}'); return false;"> {assign var='userAttendee' value=$event->getAttendee()->setDateFilter($objEventDtstart)->findAttendee($user)}
                    {if $userAttendee}
                            {if $userAttendee->getAnswer() == 'NONE'}<img src="{$AppTheme->images}/decorators/event/btnRSVP.gif" /> 
                            {elseif $userAttendee->getAnswer() == 'YES'}<img src="{$AppTheme->images}/decorators/event/btnAttending.gif" /> 
                            {elseif $userAttendee->getAnswer() == 'NO'}<img src="{$AppTheme->images}/decorators/event/btnNotAttending.gif" /> 
                            {elseif $userAttendee->getAnswer() == 'MAYBE'}<img src="{$AppTheme->images}/decorators/event/btnMaybe.gif" /> 
                            {/if}
                        {/if} 
                    </a>
            </div>

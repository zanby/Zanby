...
<table class="prForm">
	<tbody>
		...
		<tr>
			<td>
				<select class="prFloatLeft" id='display_f_type_select_{$cloneId}' onchange="display_f_type_select_change('{$cloneId}', this.selectedIndex);return false;">
		...
		<tr>
			<td>
				...
				<select class="prFloatLeft" name="default_f_index_sort_{$cloneId}" id="default_f_index_sort_1_{$cloneId}" onchange="default_f_index_sort_change('{$cloneId}', this.value);return false;">
		...
		<tr>
			<td colspan="2">
				<label class="prTBold" for="display_f_number_in_each_region_{$cloneId}">{t}Show top{/t} </label>
				...
				{if $default_index_sort == 1}
					<label class="prTBold">{t}Friends per region{/t}</label>
				{else}
					<label class="prTBold">{t}Friends{/t}</label>
				{/if}
			</td>
		</tr>
	</tbody>
...
{/if}
<!-- / -->
{include file="content_objects/ddMyFriends/light_block_wide.tpl"}

 {foreach from=$discussionsThreads item=current name=threads key=currkey}
    ...
    <div class="prInnerTop">
        ...
        <div class="prInner">
            <h4>
                <a href="{$CurrentGroup->getGroupPath('topic/topicid')|cat:$topic->getId()}/"> {$topic->getSubject()|escape:"html"} </a>
            </h4>
            {if $discussionsShowThreadSummaries && $discussion->getId()}
            <p class="prInnerSmall">
                {$topic->getTopicPost()->getTextContent()|escape:"html"}
                <a href="{$CurrentGroup->getGroupPath('topic/topicid')|cat:$topic->getId()}/"> {t}More{/t}&nbsp;&raquo;
                </a>
            </p>
            {/if}
    ...
    <div class="prInnerTop"> {if $discussionsShowThreadsNumber && ($discussionsDisplayMostActive || $discussionsDisplayMostRecent)}
        <!-- most active -->
        ...
        <div id="discussion_tab_0_{$cloneId}_{$currkey}" style="display:block;" class="prSubNav prNoBorder">
            ...
            <ul class="prClr3">
                ...
                <li>
                    <a class="prNoBorder" href="#null" onclick="switchDiscussionsTab('{$cloneId}','{$currkey}',1); return false;">{t}Most Recent{/t}</a>
                </li>
                {/if}
            </ul>
        ...
        <div id="discussion_tab_1_{$cloneId}_{$currkey}" {if $discussionsDisplayMostActive}style="display:none;"{else}style="display:block;"{/if} class="prSubNav prNoBorder">
            ...
            <ul class="prClr3">
                {if $discussionsDisplayMostActive}
                <li>
                    <a class="prNoBorder" href="#null" onclick="switchDiscussionsTab('{$cloneId}','{$currkey}',0); return false;">{t}Most Active{/t}</a>
                </li>
                {/if}
                <li class="active">

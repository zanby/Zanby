...
<div class="themeA">
    <iframe src="" id="wMap_iframe_{$cloneId}" style="width:445px; height:320px;" frameborder="0"></iframe>
</div>
    ...
    <table class="prForm">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input name="wMap_displayRange_{$cloneId}" id="wMap_displayRange_1_{$cloneId}" type="radio" class="prAutoWidth prNoBorder" {if $displayRange==1}checked="checked"{/if} onclick="wMap_displayRange_change('{$cloneId}',1);" /><label for="wMap_displayRange_1_{$cloneId}">&nbsp;{t}Just My Family{/t}</label>
                </td> 
            </tr>
            ...
            <tr {if !$eventsDisplayType}style="display:none"{/if} id="wMap_eventToDisplayId_2_{$cloneId}">
                <td colspan="4">
                    ...
                    {if $ndaCount}
                        <select id="wMap_eventToDisplayId_select_{$cloneId}" onchange="set_wMap_eventToDisplayId(this.options[this.selectedIndex].value, '{$cloneId}');return false;">

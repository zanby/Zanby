<div class="themeA prCOCentrino">
    <img width="150" class="prFloatLeft prIndentRight" src="{$video->getCover()->setWidth(385)->getImage()}" alt="{$video->getTitle()|escape:html}" title="{$video->getTitle()|escape:html}" /> 
	{if $video->getId()}
		<div class="prTLeft">                                                        
			<h3 class="prWithoutInnerTop">{$video->getTitle()|escape:html}</h3>
			{$video->getCreateDate()|user_date_format:$user->getTimezone()|date_locale:'DATE_MEDIUM'}{t} by {/t}<a href="{$video->getCreator()->getUserPath('profile')}"><strong>{$video->getCreator()->getLogin()|escape:html}</strong></a><br />

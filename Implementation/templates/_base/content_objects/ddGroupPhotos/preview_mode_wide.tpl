    ...
    {if $gallery_show_as_icons}
        <ul>
            ...
            <li class="prFloatLeft prIndentLeftSmall" id="ddGroupPhotos_{$cloneId}_{$myIteration}">
                <a href="{$currentGroup->getGroupPath('galleryView/id')|cat:$current->getId()}/">
                    <img src="{$current->setWidth(35)->setHeight(35)->getImage()}" alt="" /></a>
            </li>
            <script type="text/javascript">YAHOO.example.container.ttdocs_{$cloneId}_{$myIteration} = new YAHOO.widget.Tooltip("ttdocs_{$cloneId}_{$myIteration}", {$smarty.ldelim} hidedelay:100, width:"250px" ,context:"ddGroupPhotos_{$cloneId}_{$myIteration}", text:"<b>{$current->getTitle()|escape:'html'}</b><br>{$current->getDescription()|escape:'javascript'}<br>Shared by {$current->getCreator()->getLogin()|escape:'html'} on {$current->getCreateDate()|user_date_format:$user->getTimezone()|date_locale:'DATETIME'} {$TIMEZONE}"{$smarty.rdelim});
  				</script>
            {/if}
            {/foreach}
        </ul>
    ...
    <div class="prCOCentrino prClr3">
        ...
        <div class="prFloatLeft prIndent"> {if $current && $current->getId()}
           ...
           {* <div> {if !$currPhoto->getTagsList()}&nbsp;{/if}
                ...
                {/foreach} </div> *}
            <!-- photo -->
            <div id="ddGroupPhotosG_{$cloneId}_{$myIteration}"> {if $currPhoto->getId()}
                <a href="{$currentGroup->getGroupPath('galleryView/id')|cat:$currPhoto->getId()}/"><img class="prGrayBorder" alt="" title="" src="{$currPhoto->setWidth(119)->setHeight(89)->getImage($user)}" /></a>
                {else}&nbsp;{/if} </div>
            <!-- title -->
            <div style="overflow: hidden; width:118px; height:35px;">
                <a href="{$currentGroup->getGroupPath('galleryView/id')|cat:$currPhoto->getId()}/" title="{$currPhoto->getTitle()}">{$currPhoto->getTitle()|strip_tags}</a>
            </div>
            <script type="text/javascript">
                            YAHOO.example.container.ttdocs_{$cloneId}_{$myIteration} = new YAHOO.widget.Tooltip("ttdocs_{$cloneId}_{$myIteration}", {$smarty.ldelim} hidedelay:100, width:"250px" ,context:"ddGroupPhotosG_{$cloneId}_{$myIteration}", text:"<b>{$current->getTitle()|escape:'html'}</b><br>{$current->getDescription()|escape:'javascript'}<br>Shared by {$current->getCreator()->getLogin()|escape:'html'} on {$current->getCreateDate()|user_date_format:$user->getTimezone()|date_locale:'DATETIME'} {$TIMEZONE}"{$smarty.rdelim});
              </script>
            {/if} </div>
        {/foreach} </div>
    {/if} </div>

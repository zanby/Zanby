...
{foreach from=$discussionsThreads item=current name=threads key=currkey}
			...
			<div class="prInnerTop">
				...
				{foreach from=$discussion->getTopics()->findByDiscussionId($current[0]) item=topic}
					{if $topic->getId() == $current[1]}
						<div class="prInner">
							<h4><a href="{$CurrentGroup->getGroupPath('topic/topicid')|cat:$topic->getId()}/">
								{$topic->getSubject()|escape:"html"}
							</a></h4>
							{if $discussionsShowThreadSummaries && $discussion->getId()}
								<p class="prInnerSmall">
									{$topic->getTopicPost()->getTextContent()|escape:"html"}
									<a href="{$CurrentGroup->getGroupPath('topic/topicid')|cat:$topic->getId()}/">
					                    {t}More{/t}&nbsp;&raquo;
					                </a>
								</p>
							{/if}
							...
							<p>
								{t}{tparam value=$topic->getPostsCount()} %s posts {/t}|
								<span>{t}Posted by{/t}</span>
								<a href="{$topic->setAuthor($user->createAuthorById($author_id))->getAuthor()->getUserPath('profile')}">
				                    {$topic->setAuthor($user->createAuthorById($author_id))->getAuthor()->getLogin()|escape:"html"}
				                </a>
							</p>
	...
	{foreachelse}
		{if !$discussionsShowThreadsNumber}
			<p class="prInner">{t}You have not selected any discussions to display.{/t}</p>
		{/if}
	{/foreach}
	{assign var="currkey" value="1"}
	<div class="prInnerTop">
		{if $discussionsShowThreadsNumber && ($discussionsDisplayMostActive || $discussionsDisplayMostRecent)}
			<!-- most active -->
			{if $discussionsDisplayMostActive}
				<div id="discussion_tab_0_{$cloneId}_{$currkey}" style="display:block;" class="prSubNav prNoBorder">
					<!-- tabs -->
					<ul class="prClr3">
						<li class="active"><a class="prNoBorder" href="#">{t}Most Active{/t}</a></li>
						{if $discussionsDisplayMostRecent}
							<li><a class="prNoBorder" href="#null" onclick="document.getElementById('discussion_tab_1_{$cloneId}_{$currkey}').style.display = 'block'; document.getElementById('discussion_tab_0_{$cloneId}_{$currkey}').style.display = 'none'; return false;">{t}Most Recent{/t}</a></li>
						{/if}
					</ul>
            <div class="prClearer"></div>
					<!-- /tabs -->
					<!-- tabs area -->
					<div class="prInnerSmall prGrayBorder">
			...
			{if $discussionsDisplayMostRecent}
				<div id="discussion_tab_1_{$cloneId}_{$currkey}" {if $discussionsDisplayMostActive}style="display:none;"{else}style="display:block;"{/if} class="prSubNav prNoBorder">
					...
					<ul class="prClr3">
						{if $discussionsDisplayMostActive}
							<li><a class="prNoBorder" href="#null" onclick="document.getElementById('discussion_tab_0_{$cloneId}_{$currkey}').style.display = 'block'; document.getElementById('discussion_tab_1_{$cloneId}_{$currkey}').style.display = 'none'; return false;">{t}Most Active{/t}</a></li>
						{/if}
            ...
            <div class="prClearer"></div>
					...
					<div class="prInnerSmall prGrayBorder">
                        ...
                        {foreachelse}
                        <p>{t}There are no recent threads.{/t}</p>
                        {/foreach}
					</div>
					<!-- /tabs area -->
				</div>
	...
	<div class="prInnerTop">
      <a class="prLink2" href="{$CurrentGroup->getGroupPath('discussion')}">{t}All Discussions{/t} &raquo;</a>
    </div>
</div>
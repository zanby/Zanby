					...
					{foreach from=$current item=item key=key}
						...
						{else}
							<li class="prFloatLeft prPerWidth50 prClr3">
								<img id="gmembers_{$cloneId}_{$item->getId()}" src="{$item->getAvatar()->setWidth(35)->setHeight(35)->getImage()}" alt="" title="" class="prFloatLeft" />
	...
	{else}
		...
		<ul class="prClr3">
			...
			{foreach from=$gml->getList() item=item key=key}
				...
				{else}
					<li class="prFloatLeft prPerWidth50 prClr3">
						<img id="gmembers_{$cloneId}_{$key}" src="{$item->getAvatar()->setWidth(35)->setHeight(35)->getImage()}" alt="" title="" class="prFloatLeft" />
						<div class="prFloatLeft">
							<h4>
								<a href="{$item->getUserPath('profile')}">{$item->getLogin()}</a>
								{if $gml->isHost($item)} <strong>{t}HOST{/t}</strong>{/if}

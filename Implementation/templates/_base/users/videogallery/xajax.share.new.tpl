        ...
        <div class="prInnerTop">       
          ...
          {if $nonegroups == true}  
			...
			{t var="button"}Share Collection{/t}       
            {linkbutton name=$button link="#" onclick="xajax_share_group_do("|cat:$gallery->getId()|cat:", document.getElementById('group_id').options[document.getElementById('group_id').selectedIndex].value, '"|cat:$JsApplication|cat:"'); return false;"} 
			</div>       
          {/if}        
        {if $nonegroups != true}
		...
		{t var="button_01"}Share Collection{/t}
        {linkbutton name=$button_01 link="#" onclick="xajax_share_friend_do("|cat:$gallery->getId()|cat:", document.getElementById('friend_id').options[document.getElementById('friend_id').selectedIndex].value, '"|cat:$JsApplication|cat:"'); return false;"}
		</div>
        {/if} 
		</div>     
        {if $nonefriends != true}
        ...
        {linkbutton name=$button_02 link="#null" onclick="popup_window.close();"}
        </div>
		{*popup_item*}
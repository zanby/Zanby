...
{form_errors_summary}
<div class="prClr3">
<table cellspacing="0" cellpadding="0" border="0" class="prForm" id="calendar-add-event-block">
    <col width="25%" />
    <col width="65%" />
    <col width="10%" />
	<thead>
            ...
            <td class="prClr2">
    			<div id="EventPictureBlock"{if !$objCopyEvent->getPictureId()} style="display:none"{/if}>
    				{if $objCopyEvent->getPictureId()}
    					<img src="{$objCopyEvent->getEventPicture()->setWidth(75)->setHeight(75)->getImage($user)}" class="prFloatLeft" id="EventImageObj"/>
    				{else}
    					<img src="{$AppTheme->images}/decorators/event/no_event_image.jpg" class="prFloatLeft prIndentRightSmall" id="EventImageObj" />
    				{/if}
                        ...
                        <tbody>
                            ...
                            <tr>
                               ...
                               <td>
									...
									<div  class="prTip">{t}{tparam value=$SITE_NAME_AS_STRING}These email addresses will be stored in your {$SITE_NAME_AS_STRING} address book<br />for future use. %s will not use them for marketing purposes.{/t}</div>
                                </td>
							   <td>
							   </td>
                            </tr>
...
{linkbutton color="blue" name=$button_01 onclick="document.forms['form_add_event'].submit(); return false;" class="prFloatLeft"}</div>

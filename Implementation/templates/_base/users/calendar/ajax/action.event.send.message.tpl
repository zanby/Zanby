...
{form from=$form id="form_message_to_guest" onsubmit=$linkUrl}
	...
	<table class="prForm">
		...
		<tr>
			...
			<div class="prIndentTopSmall">
			{form_radio name="to" value="YES_MAYBY" checked=$formParams.to id="to_YES_MAYBY"} <label for="to_YES_MAYBY">{t}Guests that have RSVP'd YES or MAYBE{/t}</label>
			</div>
			<div class="prIndentTopSmall">
			{form_radio name="to" value="YES_MAYBY_NONE" checked=$formParams.to id="to_YES_MAYBY_NONE"} <label for="to_YES_MAYBY_NONE">{t}Guests that have RSVP'd YES, MAYBE or Have not responded{/t}</label>
			</div>
			</td>
		</tr>
		<tr>
			<td class="prTRight"><label for="message">{t}Message:{/t}</label></td>				
			<td>{form_textarea name="message" id="message" value=$formParams.message rows="10" style="width:100%"}</td>
		</tr>			
		<tr>				

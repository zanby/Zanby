...
<col width="85%" />
	<tr>
		<td class="prTRight prInnerTop">
		<label for="event_title">{t}Name:{/t}</label></td>
		<td>
	...
	<tr>
		<td class="prTRight prInnerTop">
		<label for="event_time_hour">{t}Time:{/t}</label></td>
		<td>
	...
	<tr>
		<td class="prTRight prInnerTop">
		<label for="event_category">{t}Category:{/t}</label></td>
		<td>
		<div class="prIndentTopSmall">
...
{t var='button'}Add Event{/t}
{form_submit name="add" value=$button onclick="xajax_doEasyAddEvent('"|cat:$year|cat:"', '"|cat:$month|cat:"', '"|cat:$day|cat:"', xajax.getFormValues('easyAddEvent'));  return false;"}</div>
{/form}

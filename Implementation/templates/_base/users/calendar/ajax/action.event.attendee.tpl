...
<span class="prInnerRight">{form_radio name="attending_rsvp_way" id="attending_rsvp_way_no" value="NO" checked=$userAttendee->getAnswer()}<label for="attending_rsvp_way_no"> {t}No{/t}</label></span>
...
<div class="prTCenter prIndentTop">
    {t var='button'}Send RSVP{/t}
    {linkbutton name=$button  onclick="xajax_doAttendeeEvent(`$event_id`, `$uid`, '`$view`', xajax.getFormValues('rsvp_event_form'), '`$date`', params); return false;"}

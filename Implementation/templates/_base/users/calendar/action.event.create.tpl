		...
		CreateEventApp = function () {
			return {
				...
				onDtstartSelected : function (type, args, obj) {
					...
					//CreateEventApp.rruleUntilCalendar.cfg.setProperty('mindate', '4/1/2008');
					//CreateEventApp.rruleUntilCalendar.render();
...
<div class="prInnerSmall prClr3">
   ...
   <table class="prForm" id="calendar-add-event-block">
    ...
    <col width="10%" />
    <thead>
        <tr><th colspan="3" class="prTLeft prText5">{t}Fields marked with asterisk <span class="prMarkRequired">*</span> are required.{/t}</th></tr>
    </thead>
    <tbody>
        <tr>
        ...
        <tr>
            ...
            <td>{form_textarea name="event_description" id="eventDescription" value=$objEvent->getDescription()|escape:html rows="5"}</td>
    		<td class="prTip"></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
    			<label for="event_time_hour_label" class="prEventTimeLabel">{t}Starts at{/t} </label>
            ...
            <td class="prClr2">
    			<div id="EventPictureBlock"{if !$objEvent->getPictureId()} style="display:none"{/if}>
    				{if $objEvent->getPictureId()}
    					<img src="{$objEvent->getEventPicture()->setWidth(75)->setHeight(75)->getImage($user)}" class="prFloatLeft" id="EventImageObj"/>
    				{else}
    			...
    			<div id="EventPictureBlockNONE"{if $objEvent->getPictureId()} style="display:none"{/if}>
    				...
    				<div class="prFloatLeft">
    					<a href="#" onclick="xajax_doAttachPhoto(YAHOO.util.Dom.get('event_picture_id').value);return false;">+  Attach Picture</a>
    				</div>
    			</div>
                    ...
                    <table class="prForm" id="invite-guests-options-full">
                        ...
                        <tbody>
                            ...
                            <tr>
                               ...
                               <td>
                                    {t}Enter an email address or a {$SITE_NAME_AS_STRING} username, separated by a comma.{/t}<br />
                                    ...
                                    {/if}
                                    <div class="prInner" style="{if !$formParams.event_invitations_lists}display:none{/if}" id="EventInviteListsObjects">
                                        {include file="users/calendar/action.event.template.contact.list.tpl"}
                                    </div>
                                    <div class="prInner" style="{if !$formParams.event_invitations_groups}display:none{/if}" id="EventInviteGroupsObjects">
                                        {include file="users/calendar/action.event.template.contact.group.tpl"}
                                    </div>
                                    <div class="prIndentTop">{form_textarea id="inv_emails" name="event_invitations_emails" value=$formParams.event_invitations_emails|escape:html}</div>
                            ...
                            <tr>
                               ...
                               <td>
                                    ...
                                    <label for="event_allow_guests_invitation">{t}Allow guests to invite other people{/t}</label><br />
                                    {form_checkbox name="event_display_guests" id="event_display_guests" value="1" checked=$formParams.event_display_guests}
                                    <label for="event_display_guests">{t}Display Guest List to Invited Guests{/t}</label>
...
<div class="prDropBox prTRight">
{t var='button_01'}Save Event{/t}
{linkbutton color="blue" name=$button_01 onclick="document.forms['form_add_event'].submit(); return false;" html=$html htmlPosition="right"}</div>

...
<div class="prMessagesButtonBlock">
	...
	{linkbutton name=$button_01 link="#" onclick="delete_selected(getMouseCoordinateX(event), getMouseCoordinateY(event));" link="#"}
</div>
	...
	{foreach item=m from=$messagelist name=loop_messagelist}
		...
		{/if}
			<tr class="{if $messageClass} {$messageClass}{/if}" id="mess_{$mid}">
				...
				<td>
					<span class="prEllipsis prMessageName" title='{$m->getRecipientsStringName()|escape:"html"}'>
						<span class="ellipsis_init">{$m->getRecipientsStringName()|escape:"html"}</span>
					</span>
				</td>
				<td><span class="prMessageSubject"><a href='{$currentUser->getUserPath("messageview/order/`$order`/id/`$mid`")}' title='{$m->getSubject()|escape:"html"}'>{$m->getSubject()|escape:"html"}</a></span></td>

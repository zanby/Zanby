...
<div id="shareFilePanelSharedGroups" class="prClr3">
	{foreach item=g from=$sharedGroupsList}
        {if $Warecorp_List_AccessManager->canUnshareList($list, $currentUser, $user)}
            {assign var="checkId" value='family_'|cat:$g->getId()}
            <div id="groupShare_{$g->getId()}_{$list->getId()}" class="prClr3 prIndentTopSmall">
                <div class="prFloatLeft prIndentRight">{t}{if $familySharedWithAliases[$checkId]}{tparam value=$familySharedWithAliases[$checkId]|escape:"html"}{else}{tparam value=$g->getName()|escape:"html"}{/if}Shared with %s{/t}</div>
...
<div class="prInnerBottom">
	...
	{linkbutton name=$button_01 link="javascript:void(0)" onclick="xajax_list_share("|cat:$list->getId()|cat:", 'group', document.getElementById('group_id').options[document.getElementById('group_id').selectedIndex].value); return false;"}
</div>	
</div>        
{else}
...
<div class="prInnerBottom">
	...
	{t var='button_03'}Share List{/t}				
	{linkbutton name=$button_03 link="javascript:void(0)" onclick="xajax_list_share("|cat:$list->getId()|cat:", 'user', document.getElementById('friend_id').options[document.getElementById('friend_id').selectedIndex].value); return false;"}
	</div>
</div>
{else}

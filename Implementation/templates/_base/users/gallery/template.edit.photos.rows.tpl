...
{foreach item=p name='photos' from=$photoslist}
	<div id="photoContent{$p->getId()}" class="prInnerBottom prClr3">			
		{if $expand == 'all'}{include file="users/gallery/template.edit.photo.edit.tpl" photo=$p}    
		{else}{include file="users/gallery/template.edit.photo.view.tpl" photo=$p}{/if}			
	</div>                              
{foreachelse}
	{t}No Photos{/t}		
{/foreach}

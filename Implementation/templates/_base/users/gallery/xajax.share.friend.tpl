						...
						<div>
							<select name="shareFriendUsers" id="shareFriendUsers" multiple="multiple" size="5" style="width:300px;">
								...
								{if $gallery->isShared($friend->getFriend())}
									<option value="{$friend->getFriend()->getId()}" disabled="disabled">{$friend->getFriend()->getLogin()|escape:"html"} - {t}{tparam value=$gallery->getShareDate($friend->getFriend())}Shared on %s{/t}</option>
								{elseif $gallery->isWatched($friend->getFriend())}
									<option value="{$friend->getFriend()->getId()}" disabled="disabled">{$friend->getFriend()->getLogin()|escape:"html"} - {t}Watching{/t}</option>
								{else}
									<option value="{$friend->getFriend()->getId()}">{$friend->getFriend()->getLogin()|escape:"html"}</option>
								{/if}								
								{/foreach}
							</select>
						</div>

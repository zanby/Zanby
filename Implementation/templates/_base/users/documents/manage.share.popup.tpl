...
<div><a href="javascript:void(0)" onclick="DocumentApplication.shareDocument('{$document->getId()}'); return false;">{t}Return{/t}</div>
		<div class="prPopupHeight"><table class="prResult"> 
        	<col width="80%" />
        	<col width="20%" />  
        	{foreach item=g from=$sharedGroupsList key=groupId}
            	{if $Warecorp_Document_AccessManager->canUnshareDocument($document, $currentUser, $user->getId())}
                    <tr>
                        <td>

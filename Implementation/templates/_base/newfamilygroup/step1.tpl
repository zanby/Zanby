...
<div class="prDropHeader">
		<!-- -->
				<h2>{t}Step 1: Basic information{/t}</h2>
</div>
<!-- Pay attention to _style attr -->
	...
	<table cellpadding="0" cellspacing="0" border="0" class="prForm">
	  ...
	  <tr>
		<td class="prTRight"><span class="prMarkRequired">*</span>
		  <label>{t}Group Family Name:{/t}</label></td>
		<td>
		{form_text name="name" value=$group.name|escape:"html" onBlur="setGemail(this);"}
		</td>
		<td></td>
	  </tr>
	  <tr>
		<td class="prTRight"><span class="prMarkRequired">*</span>
		  <label>{t}Group Family Category:{/t}</label></td>
		<td>
		{form_select name="categoryId" selected=$group.categoryId options=$categories}
		</td>
		<td></td>
	  </tr>
	  <tr>
		<td class="prTRight"><span class="prMarkRequired">*</span>
		  <label>{t}Group Family Address:{/t}</label></td>
		<td>
		{form_text name="gemail" id="gemail" value=$group.gemail|escape:"html"  dir="rtl"}
		</td>
	  ...
	  <tr>
		...
		<td>
			{form_textarea name="description" value=$group.description|escape:"html"}
		</td>
		<td class="prTip">{t}(2000 characters max){/t}</td>
	  </tr>
	  <tr>
		<td class="prTRight">
		  <label>{t}Tags:{/t}</label></td>
		<td>
		{form_text name="tags" value=$group.tags|escape:"html" }
		</td>
		<td></td>
	  </tr>
	  <tr>
		<td class="prTRight">
		  <label>{t}Name:{/t}</label></td>
		<td>
		{form_text name="company" value=$group.company|escape:"html" }
		</td>
		<td></td>
	  </tr>
	  ...
	  <tr>
		<td class="prTRight">
		  <label>{t}Position/Title:{/t}</label></td>
		<td>
		{form_text name="position" value=$group.position|escape:"html" }
		</td>
		<td></td>
	  </tr>
	  <tr>
		...
		<td>
		{form_text name="address1" value=$group.address1|escape:"html" }
		</td>
		<td></td>
	  </tr>
	  <tr>
		<td class="prTRight">
		  <label>{t}Address2:{/t}</label></td>
		<td>
		{form_text name="address2" value=$group.address2|escape:"html" }
		</td>
		<td></td>
	  </tr>
	  <tr>
		...
		<td>
		{form_select id="countryId" name="countryId" options=$countries onchange="xajax_detectCountry(this.options[this.selectedIndex].value);" selected=$group.countryId}
		</td>
		<td></td>
	  </tr>
	...
	<tr id="LocationTrZip" {if !$group.countryId || ($group.countryId != 1 && $group.countryId != 38)}style="display:none;"{/if}>
		<td class="prTRight"><span class="prMarkRequired">*</span> <label>{t}Zip code:{/t}</label></td>
		<td>
			<div class=" yui-skin-sam">
				<div class="yui-ac">
					{form_text name="zipId" id="zipId"  value=$group.zipId|escape:"html"}
					<div id="acZip"></div>
				</div>
			</div>
		</td>
	...
	<tr id="LocationTrCity" {if !$group.countryId || $group.countryId == 1 || $group.countryId == 38}style="display:none;"{/if}>
		<td class="prTRight"><span class="prMarkRequired">*</span> <label>{t}City:{/t}</label></td>
		<td>
			<div class=" yui-skin-sam">
				<div class="yui-ac">
	...
	{* CHANGES FOR LOCATIONS END *}
	  <tr>
		...
		<td>
			{form_radio id="whcj1" name="hjoin" value="0" checked=$group.hjoin|default:"0"}
			<label for="whcj1">{t}Anyone{/t}</label>
			<div class="prIndentTopSmall">
			{form_radio id="whcj2" name="hjoin" value="1" checked=$group.hjoin}
			<label for="whcj2">{t}Only those I approve{/t}</label>
			</div>
			<div class="prIndentTopSmall">
			{form_radio id="whcj3" name="hjoin" value="2" checked=$group.hjoin} 
			<label for="whcj3">{t}Only those with the following code:{/t}</label>
			</div>
		</td>
		<td></td>
	  </tr>
	  <tr>
		<td></td>
		<td>{form_text name="jcode" value=$group.jcode|escape:"html"}</td>
		<td></td>
	  </tr>
	  <tr>
	  <td></td>
	  <td colspan="2" class="prTip">{t}You can choose a word or number as your code. It will be included in the inventation you send to the people you invite to join your group.{/t}</td>
	  </tr>
	</table>
		<div class="prTCenter prIndentTop">
		{t var='button'}Continue{/t}
		{form_submit name="Continue" value=$button}</div>
	{/form}
	</div>
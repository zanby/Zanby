<?php
    ...
    {
		Warecorp::addTranslation('/plugins/function.template_discussion_ournew.php.xml');  
        $output  = "<td style='border: 1px solid black;'> ".Warecorp::t('Discussion1')."</td>";
        $output .= "<td style='border: 1px solid black;'> ".Warecorp::t('title')." - ".htmlspecialchars(strip_tags($params['object']->entityTitle()))."</td>";
        $output .= "<td style='border: 1px solid black;'> ".Warecorp::t('Decription')." - ".substr(htmlspecialchars(strip_tags($params['object']->entityDescription())), 0, 200)."</td>";

<?php
Warecorp::addTranslation('/modules/groups/xajax/lists/action.view.rank.record.php.xml');
    ...
    if (!$AccessManager->canRankRecord($record, $this->currentGroup, $this->_page->_user->getId())) {
        return;
    }
    ...
    if ( isset($rank) ) {
        ...
        if ($rank>=1 && $rank<=5) {
            ...
            $output = $this->view->getContents('groups/lists/lists.view.record.rank.tpl');

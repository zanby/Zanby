<?php
Warecorp::addTranslation('/modules/groups/videogallery/xajax/action.videogalleryUnShareGroupDo.php.xml');
...
if ( !empty($gallery) && $gallery->getId() !== null && !empty($group) && null !== $group->getId() ) {
    ...
    if ( false === $allGroupsSharing && Warecorp_Video_AccessManager_Factory::create()->canUnShareGallery($gallery, $group, $this->_page->_user) ) {
                                        
        $gallery->unshare($group, false);

        $objResponse->addScript($application.'.showShareNew(null);');
    } elseif ($allGroupsSharing && Warecorp_Video_AccessManager_Factory::create()->canUnshareGalleryToAllFamilyGroups($gallery, $group, $this->_page->_user)) {   
        ...
        $objResponse->showAjaxAlert(Warecorp::t('Gallery unshared'));
                ...
                !Warecorp_Video_AccessManager_Factory::create()->canUnshareGalleryToAllFamilyGroups($gallery, $group, $this->_page->_user) && 

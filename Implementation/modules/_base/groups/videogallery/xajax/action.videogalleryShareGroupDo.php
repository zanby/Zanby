<?php
Warecorp::addTranslation('/modules/groups/videogallery/xajax/action.videogalleryShareGroupDo.php.xml');
...
if ( $gallery && $gallery->getId() && $group && $group->getId() && Warecorp_Video_AccessManager_Factory::create()->canShareGallery($gallery, $this->currentGroup, $this->_page->_user) ) {
    ...
    /*    if ( $application == 'PGLApplication' ) {
        ...
        if ( SINGLEVIDEOMODE ) {
            $objResponse->showAjaxAlert(Warecorp::t('You can not share this video'));
        } else {
            $objResponse->showAjaxAlert(Warecorp::t('You can not share this collection'));
        }
    }
} else {

<?php
    Warecorp::addTranslation('/modules/groups/documents/xajax/action.documentDeleteGroup.php.xml');
            ...
            foreach ( $this->params['groups'] as $_docId ) {
                ...
                if ( null !== $objDocument->getId() ) {
                    if ( $objDocument->isDocumentShared($objDocument->getId(),'group', $this->currentGroup->getId()) ) {
                        ...
                        /**
                         * check if document is host kit document (zccf, zccf-base, zccf-alt)
                         * @author Artem Sukharev
                         */
                        $objRound = Warecorp_Round_Item::getCurrentRound($this->currentGroup->getId());
                        if ( $objRound->getRoundIsCurrent() && $objRound->getRoundDocumentId() == $objDocument->getId() ) {
                            $popup_window = Warecorp_View_PopupWindow::getInstance();
                            $popup_window->title(Warecorp::t('Information'));
                            $popup_window->content('<p>' . Warecorp::t("Document '%s' is Host Kit and can be removed only from Family Tools 'Survey & Round' page.", $objDocument->getName()) . '</p>');
                            $popup_window->width(350)->height(100)->reload($objResponse);
                            $objResponse->printXml($this->_page->Xajax->sEncoding);
                            exit;
                        }
                        ...
                        $this->currentGroup->sendNewDataIsUploaded( $this->_page->_user, $objDocument, "FILE", "DELETE", false, array($objDocument->getName()), "FILE" );
                    }
                }
            }
        }
            ...
            foreach ( $this->params['fgroups'] as $_folderId ) {
                ...
                if ( null !== $objFolder->getId() ) {
                    if ( !in_array($objFolder->getId(), $this->params['fgroups_accept']) && !$objFolder->isDeletable() ) {
                        ...
                        /** Send notification to host **/
                        $this->currentGroup->sendNewDataIsUploaded( $this->_page->_user, $objFolder, "FILE", "DELETE", false, array($objFolder->getName()), "FOLDER" );     
                        $Script = "";

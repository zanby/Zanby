<?php
Warecorp::addTranslation('/modules/groups/promotion/action.custombadgeuploadsave.php.xml');
    ...
    if (Warecorp_Group_Standard::isGroupExists('id', floor($_REQUEST['group']))) {
        ...
        if (isset($_FILES['Filedata']) && $_FILES['Filedata']["error"] == 0) { 
            if (filesize($_FILES["Filedata"]["tmp_name"]) > IMAGES_SIZE_LIMIT) {
                exit('badge image uploaded');                       
            }            
            $data = Warecorp_File_Item::isImage($_FILES["Filedata"]["name"], $_FILES["Filedata"]["tmp_name"]);
            if ($data !== false){
                $new_photo = new Warecorp_Group_WebBadges_Item();
                $new_photo->setGroupId($group->getId());
                $new_photo->setDescription("");
                $new_photo->save();
                $r0 = Warecorp_Image_Thumbnail::makeThumbnail($_FILES["Filedata"]["tmp_name"], UPLOAD_BASE_PATH."/upload/gallery_badge/".md5($new_photo->getId())."_orig.jpg", $data[0], $data[1], true);                
            }
        }
    }
    exit(Warecorp::t('badge image uploaded'));    
}

$objResponse = new xajaxResponse ( ) ;
if (isset($this->params['upload_type']) && ($this->params['upload_type'] == 'swfupload')) {
    $objResponse->addRedirect($this->currendGroup('webbadges'));
    ...
    //print "<script>parent.document.location = '/en/webbadges/';</script>";exit;
}
...
if (!empty($_FILES["badge_image"]) && $_FILES["badge_image"]["error"] == 0){
    if (filesize($_FILES["badge_image"]["tmp_name"]) > IMAGES_SIZE_LIMIT) {
        $errors[] = Warecorp::t("File %s is too big.  Max filesize is ",$_FILES["badge_image"]['name']).$_max_size;                      
    } else {
     
        $data = Warecorp_File_Item::isImage($_FILES["badge_image"]["name"], $_FILES["badge_image"]["tmp_name"]);
        ...
        if ($data !== false){
            ...
            $r0 = Warecorp_Image_Thumbnail::makeThumbnail($_FILES["badge_image"]["tmp_name"], UPLOAD_BASE_PATH."/upload/gallery_badge/".md5($new_photo->getId())."_orig.jpg", $data[0], $data[1], true);
            $objResponse->addRedirect($this->currendGroup('webbadges'));
            $sContentHeader = "Content-type: text/xml;";
            if ($this->_page->Xajax->sEncoding && strlen(trim($this->_page->Xajax->sEncoding)) > 0) $sContentHeader .= " charset=".$this->_page->Xajax->sEncoding;
            if (is_a($objResponse, "xajaxResponse")) $objResponse = $objResponse->getXML();
            header($sContentHeader);  
            print $objResponse; exit; 
            //print "<script>parent.document.location = '/en/webbadges/';</script>";
            //exit;
        } else {$errors[] = $_FILES["badge_image"]['name'].Warecorp::t(" file is not image");}
    }
} else {
    if (!empty($_FILES["badge_image"]["name"])) {
        switch ($_FILES["badge_image"]["error"]) {

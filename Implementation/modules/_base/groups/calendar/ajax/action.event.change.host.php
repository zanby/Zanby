<?php
Warecorp::addTranslation('/modules/groups/calendar/ajax/action.event.change.host.php.xml');
    ...
    */
    ...
    if ( !$handle ) {
        ...
        $Content = $this->view->getContents('groups/calendar/ajax/action.event.change.host.tpl');
        ...
        */
        if ( $form->validate($handle) ) {
            if ( strpos($handle['username'], '@') ) {
                $objUser = new Warecorp_User('email', $handle['username']);
            } else {
        ...
        if ( !$form->isValid() ) {
            ...
            $popup_window->title(Warecorp::t("Change Event Host"));
            $popup_window->content($Content);
            $countErrors = sizeof($form->getErrorMessages()) + sizeof($form->getCustomErrorMessages());
            $popup_window->width(450)->height(260+(40*$countErrors+10))->open($objResponse);
        }
    }

<?php
Warecorp::addTranslation('/modules/groups/calendar/action.event.attendee.print.php.xml');
...
if ( $viewMode == 'ROW' ) {
    ...
    $this->_redirect($this->currentGroup->getGroupPath('calendar.event.attendee.print').'id/'.$objRequest->getParam('id', null).'/uid/'.$objRequest->getParam('uid', null).'/year/'.$dtStart->toString('yyyy').'/month/'.$dtStart->toString('MM').'/day/'.$dtStart->toString('dd').'/');
} else {
...
$this->view->objEvent = $objCopyEvent;

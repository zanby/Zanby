<?php
Warecorp::addTranslation("/modules/users/contentblocks/ddPicture/action.uploadAvatarOK.php.xml");
...
if (isset($this->params['upload_type']) && ($this->params['upload_type'] == 'swfupload')) {
	...
	//$objResponse->addScript('alert(2);xajax_load_avatars();');
...
if (!empty($this->params['upload']) && empty($this->params['upload_type'])) {
    $errors[] = Warecorp::t("Upload failed. Each file's size must be less than %sMb", 2);
}
    ...
    if ($avatarsCount < 12){
        ...
        if ( !empty($_FILES['edit']['name']["img_$i"]) && $_FILES['edit']["error"]["img_$i"] == 0){
            if (filesize($_FILES['edit']["tmp_name"]["img_$i"]) > AVATARS_SIZE_LIMIT) {
                $errors[] = Warecorp::t("File %s is too big.  Max filesize is ", array($_FILES['edit']["name"]["img_$i"], $_max_size));
                continue;
            }

            $data = Warecorp_File_Item::isImage($_FILES['edit']["name"]["img_$i"], $_FILES['edit']["tmp_name"]["img_$i"]);
            if ($data === false) {
                $errors[] = Warecorp::t("%s file is not image", $_FILES['edit']['name']["img_$i"]);
                continue;
            }
            ...
            //create thumbnail
            $r0 = Warecorp_Image_Thumbnail::makeThumbnail($_FILES['edit']["tmp_name"]["img_$i"], UPLOAD_BASE_PATH."/upload/user_avatars/".md5($this->currentUser->getId().$new_avatar->getId())."_orig.jpg", $data[0], $data[1], true);
            $valid = true;
            $avatarsCount++;
        } else {
            if (!empty($_FILES['edit']["name"]["img_$i"])) {
                switch ($_FILES['edit']["error"]["img_$i"]) {
                    case UPLOAD_ERR_INI_SIZE:
                        $errors[] = Warecorp::t("File %s is too big. Max filesize is %s", array($_FILES['edit']['name']["img_$i"], $_max_size));

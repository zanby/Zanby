<?php       
    ...
    if (null !== $photoId ) {
        ...
        $imageFullPath = $currentImage->getPath().'_orig.jpg';
        ...
        if (file_exists($imageFullPath)) {
            ...
            //create thumbnail
            $r0 = Warecorp_Image_Thumbnail::makeThumbnail($imageFullPath, UPLOAD_BASE_PATH."/upload/user_avatars/".md5($this->_page->_user->getId().$new_avatar->getId())."_orig.jpg", 0, 0, true);   
        }
    }
    
    //$objResponse->addScript("popup_window.close();");
    if ($jsCallbackCode) {
        $objResponse->addScript($jsCallbackCode);
    } else {
 ...
 /*   die;
    ...
    $objResponse = new xajaxResponse();
        ...
        $params->id = 'attachPhoto';
        $params->width = "400px";
        $params->modal = true;
        $params->close = true;
        $params->title = Warecorp::t("Select picture from all your galleries");

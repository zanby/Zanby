<?php
    Warecorp::addTranslation("/modules/users/xajax/action.addressbookAjaxUtilites.php.xml");
    ...
    $userAddressbookContactList->setListSize(10);
...
//    $contactList = new Warecorp_User_Addressbook_ContactList();
    ...
    $existingContacts = explode(',', $existingContacts);
    $newContacts = explode(',', $newContacts);
    
/*    foreach ($contacts as $index=>&$contact){
        
        if (in_array($contact->getContactId(), $newContacts)) {
        	$contact->checked = 'true';
        }
        else $contact->checked = 'false';
        if (in_array($contact->getContactId(), $existingContacts)) {
        	$contact->disabled = '1';
        }
        else $contact->disabled = '0';
//        $objResponse->addAlert($contact->getContactId() . '  ' . $contact->checked . ' ');
    }*/
    $letters = array();
    ...
    $P = new Warecorp_Common_PagingProduct($contactsCount, 10, "#");
    ...
    $this->view->linkPaging = $P->makeAjaxLinkPaging($page, 'xajax_addressbookAjaxUtilities(\''.$maillistId.'\', \''.implode(',',$existingContacts).'\',document.getElementById(\'newContacts\').value,', ',10,\''.$orderby.'\',\''.$direction.'\', \''.$filter.'\');return false;');
    $this->view->infoPaging = $P->makeInfoPaging($page);
    
    $form = new Warecorp_Form('addContactToListForm', 'post', 'javasript:void(0);');
    
    $this->view->contacts = $contacts;
    $this->view->existingContacts = implode(',',$existingContacts);
    $this->view->newContacts = implode(',',$newContacts);
    $this->view->form = $form;
    ...
    $template = 'users/addressbook/settings.ajaxUtilities.tpl';
    $Content = $this->view->getContents ( $template ) ;
    
    $popup_window = Warecorp_View_PopupWindow::getInstance();
    $popup_window->title(Warecorp::t("Pick contacts from addressbook"));
    $popup_window->content($Content);
    $popup_window->width(450)->height(($pageSize*40+240))->open($objResponse);

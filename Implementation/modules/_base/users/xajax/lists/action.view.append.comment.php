<?php
    Warecorp::addTranslation("/modules/users/xajax/lists/action.view.append.comment.php.xml");
    ...
    if ($record->getId() && $list->getId()) {
        ...
        $form->addRule('comment', 'required', Warecorp::t('Enter please comment text'));
        ...
        if ($form->validate($params)) {
			...
			if ( FACEBOOK_USED ) {
				...
				$action_links[] = array('text' => 'View List', 'href' => $this->currentUser->getUserPath('listsview/listid/'.$list->getId()));
				 ...
				 *
				$params = array(
					'url' => $this->currentUser->getUserPath('listsview/listid/'.$list->getId()),
					'title' => $list->getTitle(), 
					'orgurl' => BASE_URL,

<?php
    ...
    Warecorp::addTranslation("/modules/users/xajax/lists/action.add.change.type.php.xml");
        ...
        if (isset($_SESSION['list_new']['records']) && is_array($_SESSION['list_new']['records'])) {
            foreach ($_SESSION['list_new']['records'] as $id=>&$record) {
                if (isset($record['data']['item_fields'])) {
                    ...
                    if (strlen($record['data']['item_entry']) > 1024) {
                        $record['errors']['entry'] = Warecorp::t('"Entry" field too long (max %s)', 1024);
                    }
                    if (strlen($record['data']['item_tags']) > 255) {

<?php
    Warecorp::addTranslation("/modules/users/xajax/lists/action.add.save.php.xml");
    ...
    if ( $record_id !== null && isset($_SESSION['list_new'])) {
        ...
        if ( isset($list_new['records']) && isset($list_new['type']) && count($list_new['records']) ) {
            ...
            foreach ($list_new['records'] as $record_id=>&$record) {
                if (mb_strlen($data['item_entry'], 'UTF-8') > 1024) {
                    $record['errors']['entry'] = Warecorp::t('"Entry" field too long (max %s)', 1024);
                }
                if (isset($data['item_fields']['description']) && mb_strlen($data['item_fields']['description'], 'UTF-8') > 1024) {
                    $record['errors']['entry'] = Warecorp::t('"Entry" field too long (max %s)', 1024);
                }
                if (mb_strlen($data['item_tags'], 'UTF-8') > 255) {
                    $record['errors']["tags"] = Warecorp::t('"Tags" field too long (max %s)', 255);
                }

<?php
    Warecorp::addTranslation('/modules/users/documents/xajax/action.documentManageSharing.php.xml');
    ...
    if ( !empty($this->params['ownerType']) && !empty($this->params['ownerId']) ) {
        ...
        if ( !empty($owner) && $owner->getId() && Warecorp_Document_AccessManager_Factory::create()->canUnshareDocument($objDocument, $owner, $this->_page->_user->getId()) ) {
            if ( $allGroupsSharing && Warecorp_Document_AccessManager_Factory::create()->canUnshareDocumentToAllFamilyGroups($objDocument, $owner, $this->_page->_user) ) {
                $objDocument->unshareDocument($this->params['ownerType'], $owner->getId(), true);
            }
            elseif ( !$allGroupsSharing ) {
                $objDocument->unshareDocument($this->params['ownerType'], $owner->getId(), false);
            }
        }
    }
    ...
    $content = $this->view->getContents('users/documents/manage.share.popup.tpl');

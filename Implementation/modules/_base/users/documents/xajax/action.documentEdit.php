<?php
    Warecorp::addTranslation('/modules/users/documents/xajax/action.documentEdit.php.xml');    
    ...
    switch ( $this->params['itemType'] ) {
        ...
        case 'weblink' :
            ...
            if ( $objDocument->isDocumentShared($objDocument->getId(), 'user', $objOwner->getId()) ) {
                $popup_window = Warecorp_View_PopupWindow::getInstance();        
                $popup_window->title(Warecorp::t('Information'));
    ...
    if ( $isHandler ) {
        ...
        if ( null !== $objDocument ) {
            ...
            if ( FACEBOOK_USED ) {
                ...
                $action_links[] = array('text' => 'View Document', 'href' => $this->currentUser->getUserPath('documents/'));

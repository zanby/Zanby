<?php
    Warecorp::addTranslation('/modules/users/documents/xajax/action.documentCreateFolder.php.xml');
    ...
    /* Check permissions */
    if ( false == Warecorp_Document_AccessManager_Factory::create()->canCreateFolders($this->currentUser, $objOwner, $this->_page->_user->getId()) ) {
        $popup_window = Warecorp_View_PopupWindow::getInstance();        
        $popup_window->title(Warecorp::t('Information'));
    ...
    /* reload documents tree */
        ...
        $Script .= "node_".$folder->getId()." = new YAHOO.widget.TextNode(tmpObj, ". (($folder_id === null)?"tree_0_root_node_user_".$owner_id."":"node_".$folder_id) .", true);";
        $Script .= 'node_'.$folder->getId().'.labelStyle = "";';
        $Script .= (($folder_id === null)?"tree_0_root_node_user_".$owner_id."":"node_".$folder_id).'.refresh();';

<?php
    Warecorp::addTranslation('/modules/users/documents/xajax/action.documentSort.php.xml');
    ...
    $owner = new Warecorp_User('id', $curr_owner_id);
    ...
    if ( $curr_folder_id !== null && !$folder->getId()) {
        ...
        return;
    } elseif ( empty($this->currentUser) || !$this->currentUser->getId() ) {
    ...
    //  Check permissions        
    //*******************************************
    if (!$AccessManager->canViewOwnerDocuments($this->currentUser, $owner, $this->_page->_user->getId())) {

<?php
Warecorp::addTranslation('/modules/users/calendar/action.event.attendee.download.php.xml');
...
if ( $viewMode == 'ROW' ) {
    ...
    $this->_redirect($this->currentUser->getUserPath('calendar.event.attendee.download').'id/'.$objRequest->getParam('id', null).'/uid/'.$objRequest->getParam('uid', null).'/year/'.$dtStart->toString('yyyy').'/month/'.$dtStart->toString('MM').'/day/'.$dtStart->toString('dd').'/');
} else {

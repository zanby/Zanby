<?php
    ...
    Warecorp::addTranslation("/modules/users/calendar/action.event.create.php.xml");
    ...
    if ( false == Warecorp_ICal_AccessManager_Factory::create()->canCreateEvent($this->currentUser, $this->_page->_user) ) {
	...
	* Init Form Object
	*/
	$form = new Warecorp_Form('form_add_event', 'POST', $this->currentUser->getUserPath('calendar.event.create'));
		...
		if ( $form->validate( $objRequest->getParams() ) ) {
            ...
            */
            if ( 2 == $objRequest->getParam('event_reminder_mode') ) {
                ...
                if ( $objRequest->getParam('event_reminder_2') && $objRequest->getParam('event_reminder_2') != $objRequest->getParam('event_reminder_1')) {
                    $objReminder = new Warecorp_ICal_Reminder();
    ...
    */
    ...
    $formParams['event_event_type_1']           = ( $objRequest->getParam('event_event_type_1') )                       ? $objRequest->getParam('event_event_type_1')           : 67; // 67 is identifier of 'Personal' event cathegory
    ...
    //print_r($formParams['event_invitations_lists']);
    /**
    * Restore Documents
    */
	...
	*/
	...
	$this->view->bodyContent = 'users/calendar/action.event.create.tpl';

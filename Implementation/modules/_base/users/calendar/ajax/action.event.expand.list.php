<?php
    Warecorp::addTranslation("/modules/users/calendar/xajax/action.event.expand.list.php.xml");
    ...
    if ( sizeof($lstLists) != 0 ) {
        ...
        foreach ( $lstLists as &$objCurrList ) {
            if ( $objCurrList->getId() == $listId ) {
                ...
                $content = $this->view->getContents('users/calendar/action.event.view.template.list.expanded.tpl');  
                ...
                $content = $this->view->getContents('users/calendar/action.event.view.template.list.collapsed.tpl');
                $objResponse->addCreate("lists_list", "div", "list_{$objCurrList->getId()}");

<?php
Warecorp::addTranslation('/modules/users/calendar/action.event.attendee.print.php.xml');
...
if ( $viewMode == 'ROW' ) {
    ...
    $this->_redirect($this->currentUser->getUserPath('calendar.event.attendee.print').'id/'.$objRequest->getParam('id', null).'/uid/'.$objRequest->getParam('uid', null).'/year/'.$dtStart->toString('yyyy').'/month/'.$dtStart->toString('MM').'/day/'.$dtStart->toString('dd').'/');
} else {
...
if (!$allow) {
    $this->view->errorMessage = Warecorp::t('Sorry, you can not print the list of attendee');
    $this->view->bodyContent = 'users/calendar/action.event.error.message.tpl';
    return;
}

$attendeeList = $objEvent->getAttendee()->setFetchMode('object');
...
$this->view->attendeeYesItems = $attendeeYesItems;
$this->view->attendeeYesItemsCount = count($attendeeYesItems);
$this->view->attendeeNoItems = $attendeeNoItems;
$this->view->attendeeNoItemsCount = count($attendeeNoItems);
$this->view->attendeeMaybeItems = $attendeeMaybeItems;
$this->view->attendeeMaybeItemsCount = count($attendeeMaybeItems);
$this->view->attendeeNoneItems = $attendeeNoneItems;
$this->view->attendeeNoneItemsCount = count($attendeeNoneItems);

$this->view->bodyContent = 'groups/calendar/action.event.attendee.print.tpl';
$this->view->setLayout('main_print2.tpl');

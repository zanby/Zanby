<?php
    Warecorp::addTranslation("/modules/users/calendar/action.event.edit.php.xml");
    ...
    * Check Access
    */
    if ( false == Warecorp_ICal_AccessManager_Factory::create()->canManageEvent($objEvent, $this->currentUser, $this->_page->_user) ) {
    ...
    */
    if ( $viewMode == 'ROW' ) {
        $formURL = $this->currentUser->getUserPath('calendar.event.edit/id/'.$objRequest->getParam('id').'/uid/'.$objRequest->getParam('uid'));
    } elseif ( 'future' == $objRequest->getParam('mode', null) ) {
        $formURL = $this->currentUser->getUserPath('calendar.event.edit/id/'.$objRequest->getParam('id').'/uid/'.$objRequest->getParam('uid').'/year/'.$objRequest->getParam('year').'/month/'.$objRequest->getParam('month').'/day/'.$objRequest->getParam('day').'/mode/future');
    } else {
        $formURL = $this->currentUser->getUserPath('calendar.event.edit/id/'.$objRequest->getParam('id').'/uid/'.$objRequest->getParam('uid').'/year/'.$objRequest->getParam('year').'/month/'.$objRequest->getParam('month').'/day/'.$objRequest->getParam('day'));
    }
    ...
    */
    ...
    $this->view->bodyContent = 'users/calendar/action.event.edit.tpl';
    $this->view->friendsAssoc = $this->_page->_user->getId() ? $this->currentUser->getFriendsList()->returnAsAssoc()->getList() : array() ;

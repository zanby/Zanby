...
// 	pcntl_signal(SIGALRM, SIG_IGN);
        ...
        if ($pid == 0) {
            ...
            $it = 0;
            while (true) {
                if ($this->_createIPCsegment()) break;
                if ($it > 10) die("Couldn't open SHM segment");
                $it++;
                usleep(1000);
            }
            ...
            $it = 0;
            while (true) {
                if ($this->_createIPCsemaphore()) break;
                if ($it > 10) die("Couldn't open IPC semaphore");
                $it++;
                usleep(1000);
            }
            $this->_ipc_is_ok = true;
...
/*	    pcntl_signal(SIGALRM, array($this, "_sig_handler")); 
	    ...
	    $this->setVariable('_has_finished', true);	    
            ...
            if (!$this->_createIPCsegment() || !$this->_createIPCsemaphore()) {
                //Error create IPC segments, stop
                $this->stop();
            }
        }
    }
    ...
    function stop()
    {
        ...
        if ($this->_pid > 0) {
            ...
            while ($wait < 10) {
                $wp = pcntl_waitpid ($this->_pid, $temp = 0, WNOHANG);
                if ($wp != 0) {
            ...
            if (!$exited) {
                ...
                pcntl_waitpid ($this->_pid, $temp = 0);
                $success = pcntl_wifexited ($temp);
            }
...
// 			pcntl_alarm(1);
    ...
    function _readFromIPCsegment()
    {
        if ($this->_ipc_is_ok) {
            ...
            if (!$serialized_IPC_array)
                die("Fatal exception reading SHM segment (shmop_read)\n");
            // shmop_delete($this->_internal_ipc_key);
    ...
    function _createIPCsegment()
    {
        if ($this->_pid > 0) {
            ...
            $this->_internal_ipc_key = @shmop_open($shm_key, "c", 0666, 4096);
            ...
            $this->_internal_ipc_key = @shmop_open($shm_key, "w", 0, 0);
            if (!$this->_internal_ipc_key) {
                return false;
            }
            return true;
    ...
    function _createIPCsemaphore()
    {
        if ($this->_pid > 0) {
            ...
            $this->_internal_sem_key = @shmop_open($sem_key, "c", 0666, 10);
            ...
            if ($sem_key == -1)
                die ("Fatal exception creating semaphore (ftok)");
            $this->_internal_sem_key = @shmop_open($sem_key, "w", 0, 0);
            if (!$this->_internal_sem_key) {
                return false;
            }
            return true;

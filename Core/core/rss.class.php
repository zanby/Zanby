 ...
 * Version string.
 **/
define("FEEDCREATOR_VERSION", "ZanbyRSS 0.1");
...
class FeedItem extends HtmlDescribable {
     ...
     */
    var $author, $authorEmail, $image, $category, $comments, $guid, $source, $creator, $atomURL;
...
class FeedCreator extends HtmlDescribable {
    ...
    */
    ...
    function _createXmlns() {
        $output = "";
        if ($this->xmlns) {
            foreach ($this->xmlns as $key => $value) {
                $output .= " xmlns:$key=\"$value\"";
            }
        }
        return $output;
    }
     ...
     */
    function saveFeed($filename="", $displayContents=true, $filepath = "rss") {

        $filename = "data.xml";
        if ($filename=="") {
            $filename = $this->_generateFilename();
        }
        $feedFile = fopen($filepath . "/" . $filename, "w+");
        if ($feedFile) {
            ...
            if ($displayContents) {
                ...
                $this->_redirect($filepath . "/" . $filename);
            }
        } else {
...
class RSSCreator10 extends FeedCreator {
    ...
    function createFeed() {
        ...
        //if ($this->cssStyleSheetcssStyleSheet=="") {
...
class RSSCreator091 extends FeedCreator {
    ...
    function createFeed() {
        ...
        $feed.= $this->_createGeneratorComment();
        $feed.= $this->_createStylesheetReferences();
        $feed.= "<rss version=\"".$this->RSSVersion."\"".$this->_createXmlns().">\n";
        $feed.= "    <channel>\n";
        if ($this->atomURL !== null){
            $feed.= "        <atom:link href=\"".$this->atomURL."\" rel=\"self\" type=\"application/rss+xml\" />\n";
        }            
...
class RSSCreatorRRR extends RSSCreator091 {

    function RSSCreatorRRR() {
        parent::_setRSSVersion("RRR0.1");
    }
...
//----------------------RRR------------------------------------
class RRRCreator extends FeedCreator {
     ...
     * Contains HTML to be output at the start of the feed's html representation.
     */
    var $header = '<?xml version="1.0" encoding="ISO-8859-1"?><!-- generator="ZanbyRSS 0.1" -->';
    ...
    function createFeed() {
        ...
        {
            usort($this->items, "cmp10"); //sort by states
            $current_state = "";
        }
        ...
        {
            if ($this->display_type == 1){
                if ($current_state != $item->state && $current_state != "") $text .="</".$current_state.">\r\n";
                if ($current_state != $item->state) $text .="<".$item->state.">\r\n";
                $current_state = $item->state;
            }

            $item->memberscount = floor($item->memberscount);

            $text.=" <group>\r\n";
            $text.="  <name>".htmlspecialchars($item->title)."</name>\r\n";
            $text.="  <host>".$item->host."</host>\r\n";
            $text.="  <city>".$item->city."</city>\r\n";
            $text.="  <state>".$item->state."</state>\r\n";
            $text.="  <zipcode>".$item->postal_code."</zipcode>\r\n";
            $text.="  <description>".htmlspecialchars($item->description)."</description>\r\n";
            $text.="  <memberscount>".$item->memberscount."</memberscount>\r\n";
            $text.="  <summary>".$item->link."</summary>\r\n";

            if ($item->events){
                $event = $item->events[0];
                $text.="<event>";
                $text.="<title>".htmlspecialchars($event->title)."</title>\r\n";
                $text.="<date>".$event->format_start_date."</date>\r\n";
                $text.="<time>".$event->format_start_time."</time>\r\n";
                $text.="<description>".htmlspecialchars($event->notes)."</description>\r\n";
                $text.="<url>".$event->BrifURL."calendar/view/event/".$event->nid."</url>\r\n";
                $text.="</event>";
            }
        ...
        if ($this->display_type == 1) $text .= "</$current_state>";
        $text .="</groups>";
...
class FeedManager {
    public static function getEvents($arrEvents, $currentTimezone, $context = NULL, $itemLinkPrefix = '') {
        ...
        $rss->title = $name . " calendar ";
        $rss->description = $name . " calendar events ";
        $rss->copyright = "Copyright &copy; 2007, Zanby";
        $rss->xmlns['z'] = 'http://zanby.com/zrss#';
        
        $itemLinkPrefix2 = "http://" . $_SERVER['HTTP_HOST'] . "/" . "en/calendar.event.view/id/";
        //krsort($arrEvents);
        ...
        foreach ($arrEvents as $event){
            ...
            if ($itemLinkPrefix) {
                $path = $event->getOwner()->getPath();
                $itemLinkPrefix2 = "http://" . $path . "." . $itemLinkPrefix . "." . $_SERVER['HTTP_HOST'] . "/" . "en/calendar.event.view/id/";    
            }
            
            $item->link = $itemLinkPrefix2 . $event->getId() . "/uid/".$event->getUid()."/";
            ...
            if ($event->isAllDay()) {
                ...
                $item->description .= ' '.$objEventDtstartLocal->toString('h:mm').$objEventDtstartLocal->get(Zend_Date::MERIDIEM).' - '.$objEventDtendLocal->toString('h:mm').$objEventDtendLocal->get(Zend_Date::MERIDIEM);
            ...
            if ($venue !== null) {
                ...
                $item->additionalElements['z:longitude'] = $venue->getCity()->getLongitude();
                $item->additionalElements['z:latitude'] = $venue->getCity()->getLatitude();                
            } else {
...
{
$text.=" <group>\r\n";
$text.="  <title>".htmlspecialchars($item->title)."</title>\r\n";
$text.="  <link>".$item->link."</link>\r\n";
$item->memberscount = floor($item->memberscount);
...
$text.="  <memberscount>".$item->memberscount."</memberscount>\r\n";
$text.="<members>\r\n";
foreach ($item->memberslist as $member){
$text.="<member>\r\n";
$text.="<name>".htmlspecialchars($member["name"])."</name>\r\n";
//$text.="<email>".$member["mail"]."</email>\r\n";
$text.="</member>\r\n";
}
$text.="</members>\r\n";
}
...
$text.="    <title>".htmlspecialchars($event->title)."</title>\r\n";
$text.="    <link>".$event->BrifURL."calendar/view/event/".$event->nid."</link>\r\n";
$text.="    <date>".$event->format_start_date." ".$event->format_start_time."</date>\r\n";
$text.="    <description>".htmlspecialchars($event->notes)."</description>\r\n";
$text.="   </event>\r\n";
}
$text.="  </events>\r\n";

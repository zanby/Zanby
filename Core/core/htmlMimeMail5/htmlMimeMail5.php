...
class htmlMimeMail5
{
    ...
    */
    public function setSMTPParams($host = null, $port = null, $helo = null, $auth = null, $user = null, $pass = null, $timeout = null, $socket_set_timeout = null)
    {
        ...
        if (!is_null($timeout)) $this->smtp_params['timeout']   = $timeout;

        if (!is_null($socket_set_timeout)) $this->smtp_params['socket_set_timeout'] = $socket_set_timeout;
        else $this->smtp_params['socket_set_timeout'] = 5;
    }
    ...
    private function build()
    {
        if (!empty($this->html_images)) {
            foreach ($this->html_images as $value) {
                ...
                $this->html = preg_replace("#background:url\('$quoted'\)#", "background:url('cid:$cid')", $this->html);
            }
        }
    ...
    * @return mixed
    */
    public function send($recipients, $type = 'mail', $rebuilt = false)
    {
        if (!defined('CRLF')) {
            $this->setCRLF( ($type == 'mail' OR $type == 'sendmail') ? "\n" : "\r\n");
        }

        if ( !$this->is_built || $rebuilt ) {
            $this->build();
        }
    ...
    */
    public function getRFC822($recipients, $type = 'mail', $rebuilt = false)
    {

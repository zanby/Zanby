 ...
 * @version 1.0
 */
class BaseWarecorp_Photo_Gallery_Factory
{
	/**
	 * load Gallery object
	 * @param galleryId
	...
	public static function loadById($galleryId)
	{
		...
		$query = $db->select();
		$query->from(Warecorp_Photo_Gallery_Abstract::$_dbTableName, array('owner_id', 'owner_type'));
		$query->where('id = ?', $galleryId);
		$res = $db->fetchRow($query);
		if ( $res ) {
			switch ( $res['owner_type'] ) {
				case 'user'     : return new Warecorp_Photo_Gallery_User($galleryId);   break;
				case 'group'    : {
					$group = Warecorp_Group_Factory::loadById($res['owner_id']);
					if ( $group instanceof Warecorp_Group_Simple ) {
                        return new Warecorp_Photo_Gallery_Simple($galleryId);
					} elseif ( $group instanceof Warecorp_Group_Family ) {
                        return new Warecorp_Photo_Gallery_Family($galleryId);
					}
				    break;
				}  				

 ...
 * @version 1.0
 */
class BaseWarecorp_Photo_Gallery_List_User extends Warecorp_Photo_Gallery_List_Abstract
{
	/**
	 * id of user
	 */
	...
	public function getCount()
	{
        ...
        $query->from(array('view' => Warecorp_Photo_Gallery_Abstract::$_dbViewName), new Zend_Db_Expr('COUNT(DISTINCT view.id)'));
        if ( $this->getWhere() ) $query->where($this->getWhere());
        if ($this->withComments !== null) {
        	$tempquery = $this->_db->select()->distinct();
        	$tempquery->from(array('zuc' => 'zanby_users__comments'), array())
...
//            ->orWhere('(zgi.creator_id = ?))', $this->userId)
        ...
        */
        ...
        $sql1 = $this->_db->select()
            ...
            ->joinLeft(array('zgw' => 'zanby_galleries__watching'), 'zgw.gallery_id = zgi.id', array())
            ->where('ztr.entity_type_id = ?', 4)

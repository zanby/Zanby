...
/**
 * @package    Warecorp_Group_WebBadges
 * @copyright  Copyright (c) 2007
 * @author Evgeny Kirdzei
 */
class BaseWarecorp_Group_WebBadges_List extends Warecorp_Abstract_List 
{
    private $groupId;
    ...
    public function getList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from('zanby_groups__badge_galleries', $fields);  
        } else {
            $query->from('zanby_groups__badge_galleries');
        }
        ...
        if ( $this->isAsAssoc() ) {
            ...
            foreach ( $items as &$item ) $item = new Warecorp_Group_WebBadges_Item($item);
        }
        return $items;          
    }
    ...
    {
        $query = $this->_db->select();
        $query->from('zanby_groups__badge_galleries', new Zend_Db_Expr('COUNT(id)'))

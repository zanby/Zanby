...
/**
 * Class implement methods for LOCATIONS (City List, Country List, State List) of MEMBER any family group
 * @package Warecorp_Group_Family
 * @copyright Copyright (c) 2007
 * @author Artem Sukharev
 */
class BaseWarecorp_Group_Family_Member_LocationList extends Warecorp_Group_Location_Member_AbstractList 
{
    /**
    ...
    public function getCountriesList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('DISTINCT zlcr.id'));
        }
    ...
    {
        $query = $this->_db->select();
        $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('COUNT(DISTINCT zlcr.id)'));
        $query->joininner(array('zua' => 'zanby_users__accounts'), 'zua.id = vfu.user_id');
        $query->joinleft(array('zlc' => 'zanby_location__cities'), 'zua.city_id = zlc.id');
        $query->joinleft(array('zls' => 'zanby_location__states'), 'zlc.state_id = zls.id');
        $query->joinleft(array('zlcr' => 'zanby_location__countries'), 'zls.country_id = zlcr.id');
    ...
    public function getStatesList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('DISTINCT zls.id'));
        }
        $query->joininner(array('zua' => 'zanby_users__accounts'), 'zua.id = vfu.user_id');
        $query->joinleft(array('zlc' => 'zanby_location__cities'), 'zua.city_id = zlc.id');
        $query->joinleft(array('zls' => 'zanby_location__states'), 'zlc.state_id = zls.id');
    ...
    {
        $query = $this->_db->select();
        $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('COUNT(DISTINCT zls.id)'));
        $query->joininner(array('zua' => 'zanby_users__accounts'), 'zua.id = vfu.user_id');
        $query->joinleft(array('zlc' => 'zanby_location__cities'), 'zua.city_id = zlc.id');
        $query->joinleft(array('zls' => 'zanby_location__states'), 'zlc.state_id = zls.id');
    ...
    public function getCitiesList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('DISTINCT zlc.id'));
        }
        $query->joininner(array('zua' => 'zanby_users__accounts'), 'zua.id = vfu.user_id');
        $query->joinleft(array('zlc' => 'zanby_location__cities'), 'zua.city_id = zlc.id');
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $items = $this->_db->fetchCol($query);
            foreach ( $items as &$item ) $item = Warecorp_Location_City::create($item);
        }
        return $items;
    }
    ...
    {
        $query = $this->_db->select();
        $query->from(array('vfu' => 'view_family__users'), new Zend_Db_Expr('COUNT(DISTINCT zlc.id)'));
        $query->joininner(array('zua' => 'zanby_users__accounts'), 'zua.id = vfu.user_id');
        $query->joinleft(array('zlc' => 'zanby_location__cities'), 'zua.city_id = zlc.id');
        if ( $this->getCountryIdsIn() || $this->getCountryIdsOut() ) {
            $query->joinleft(array('zls' => 'zanby_location__states'), 'zlc.state_id = zls.id');

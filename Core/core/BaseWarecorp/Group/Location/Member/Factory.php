 ...
 */
class BaseWarecorp_Group_Location_Member_Factory
{
    static public function create($group)
    {
        if ( $group instanceof Warecorp_Group_Simple ) {
            return new Warecorp_Group_Simple_Member_LocationList($group->getId());
        } elseif ( $group instanceof Warecorp_Group_Family ) {
            return new Warecorp_Group_Family_Member_LocationList($group->getId());
        } else {

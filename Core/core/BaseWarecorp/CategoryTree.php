...
class BaseWarecorp_CategoryTree
{
	public $_db;
     ...
     * @param int $limitSet
     */
	public function __construct($handle_id = null, $table = 'category_tree', $condition = null, $limitStart = null, $limitSet = null)
	{
	  /*TODO check handle id*/ 
    ...
    public function buildTree($node = "full", $what = "*", $including = true){
        ...
        if ($node == "full"){
            $select = $this->_db->select()->from($this->getTable(), 'id')->where('parent IS NULL')->where('handle_id = '.$this->handleId);
    ...
    public function getTree($node = 1, $what = "*", $including = true)
    {
        ...
        $select->from($this->getTable(), $what)->where('lft BETWEEN :left AND :right AND handle_id = '.$this->handleId);
        if ( !$including ) {
     ...
     */
    public function getPath($node, $what = "id", $including = true, $dropRoot = false)
    {
        //get this node first
        ...
        if($including){
            $sql = "SELECT " . $what . " FROM " . $this->getTable() . " WHERE lft <= " . $node['lft'] . " AND rgt >= " . $node['rgt'].' AND handle_id = '.$this->handleId;
            if ( $this->getCondition() ) $sql .= ' AND ' . $this->getCondition();
    ...
    private function traverse($parent = 1, $left = 1, $level = 0)
    {
        ...
        $sql = "SELECT id FROM " . $this->getTable() . " WHERE parent = " . $parent.' AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND ' . $this->getCondition();
        ...
        $sql = "UPDATE " . $this->getTable() . " SET lft = " . $left . ", rgt = " . $right . ", level = " . $level . " WHERE id = " . $parent . " AND handle_id = ".$this->handleId;
        $this->_db->query($sql);
    ...
    {
        if( $parent == null ){
            $select = $this->_db->select()->from($this->getTable(), array('max' => new Zend_Db_Expr('MAX(rgt)')))->where('handle_id = '.$this->handleId);
            ...
            $sql = 'UPDATE ' . $this->getTable() . ' SET rgt = rgt + 2 WHERE rgt >= :right AND handle_id = '.$this->handleId;
            if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
            $result = $this->_db->query($sql, array('right' => $node['rgt']));

            $sql = 'UPDATE ' . $this->getTable() . ' SET lft = lft + 2 WHERE lft > :right AND handle_id = '.$this->handleId;
            if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
            $result = $this->_db->query($sql, array('right' => $node['rgt']));
    ...
    public function remove($node)
    {
        ...
        //delete node
        $sql = 'DELETE FROM  ' . $this->getTable() . ' WHERE rgt <= :right AND lft >= :left AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
        $result = $this->_db->query($sql, array('right' => $node['rgt'], 'left' => $node['lft']));

        $sql = 'UPDATE ' . $this->getTable() . ' SET rgt = rgt - (? - ? + 1) WHERE rgt > ? AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
        $sql = $this->_db->quoteInto($sql, $node['rgt'], $node['lft'], $node['rgt'] );
        $this->_db->query($sql);


        $sql = 'UPDATE ' . $this->getTable() . ' SET lft = lft - (? - ? + 1) WHERE lft > ? AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
    ...
    public function removeChildren($node)
    {
        ...
        $sql = 'UPDATE ' . $this->getTable() . ' SET rgt = rgt - (? - ? + 1) + 2 WHERE rgt >= ? AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
        $sql = $this->_db->quoteInto($sql, $node['rgt'], $node['lft'], $node['rgt'] );
        $this->_db->query($sql);


        $sql = 'UPDATE ' . $this->getTable() . ' SET lft = lft - (? - ? + 1) + 2 WHERE lft > ? AND handle_id = '.$this->handleId;
        if ( $this->getCondition() ) $sql .= ' AND '.$this->getCondition();
        $sql = $this->_db->quoteInto($sql, $node['rgt'], $node['lft'], $node['lft'] );
        $this->_db->query($sql);
    ...
    public function move($source, $destination)
    {
        ...
        if($destination['lft'] < $source['lft']) {	# left
            ...
            . ') '
            . 'WHERE lft BETWEEN '.$destination['lft'].' AND '.$source['rgt'].' AND handle_id = '.$this->handleId;
            if ( $this->getCondition() ) $sql .= ' AND ' . $this->getCondition();
        } else { #right
            $sql = 'UPDATE ' . $this->getTable() . ' SET '
            .'level = IF(lft BETWEEN '.$source['lft'].' AND '.$source['rgt'].', level -'. $source['level'] . ' + 1 + ' . $destination['level'] . ', level), '
            ...
            . ') '
            . 'WHERE lft BETWEEN '.$source['lft'].' AND '.$destination['rgt'].' AND handle_id = '.$this->handleId;
            if ( $this->getCondition() ) $sql .= ' AND ' . $this->getCondition();
        }


        //change parent reference
        $updateSql = 'UPDATE ' . $this->getTable() . ' SET parent = :destination WHERE id = :source AND handle_id = '.$this->handleId;
    ...
    public function swap($node1, $node2)
    {
        ...
        if($node1 and $node2 and $node1['parent'] == $node2['parent']){	//bogus check
            ...
            .'WHERE lft BETWEEN '.$node1['lft'].' AND '.$node2['rgt'].' AND handle_id = '.$this->handleId;

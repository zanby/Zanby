...
class BaseWarecorp_ICal_Event_List_Recurrence extends Warecorp_ICal_List_Abstract 
{
	private $event;
	...
	public function getList()
	{
		...
		if ( $this->getFetchMode() == Warecorp_ICal_List_Enum_FetchMode::OBJECT ) {
			...
			$query->where('event_uid = ?', $this->getEvent()->getId());
    ...
    public function deleteFutureRecurrence($strDate)
    {
        $where = $this->DbConn->quoteInto('event_uid = ?', $this->getEvent()->getId());
        $where .= ' AND event_recurrence_id IS NOT NULL';
        $where .= ' AND '.$this->DbConn->quoteInto('event_recurrence_id >= ?', $strDate);

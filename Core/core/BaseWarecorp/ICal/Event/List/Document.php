...
class BaseWarecorp_ICal_Event_List_Document extends Warecorp_ICal_List_Abstract 
{
    private $event;
    private $add;
     ...
     */
    protected $_pairsModeValue = 'document_id';
    
    /**
     * data base fields for assoc select
     */
    protected $_assocFields = array('id', 'document_id');
...
//        return $result;
    ...
    public function getList()
    {
        ...
        if ( $this->getFetchMode() == Warecorp_ICal_List_Enum_FetchMode::OBJECT ) {
            $query->from('calendar_event_documents', array('document_id'));
            $query->where('event_id = ?', $this->getEvent()->getId());
            $result = $this->DbConn->fetchCol($query);
            if ( sizeof($result) != 0 ) {
                foreach ( $result as &$item ) $item = new Warecorp_Document_Item($item);
            }
            ...
            */
            ...
            $query->from('calendar_event_documents', array($this->getPairsModeKey(), $this->getPairsModeValue()));
            $query->where('event_id = ?', $this->getEvent()->getId());
    ...
    * 
    */
    public function add(Warecorp_Document_Item $objDocument)
    {
        $this->add[] = $objDocument;
    }
    ...
    */
    ...
    {    
        ...
        if ( null !== $this->add && sizeof($this->add) != 0 ) {
            foreach ( $this->add as $objDocument ) {
                $data = array();
                $data['event_id']       = $this->getEvent()->getId();
                $data['event_uid']      = $this->getEvent()->getUid();
                $data['event_root_id']  = $this->getEvent()->getRootId();
                $data['document_id']    = $objDocument->getId();
                $this->DbConn->insert('calendar_event_documents', $data);
            }
        }
    }
    ...
    */
    public function deleteEventsAll()
    {
        ...
        $this->DbConn->delete('calendar_event_documents', $where);
    }

...
class BaseWarecorp_ICal_Event_List_List extends Warecorp_ICal_List_Abstract 
{
    private $event;
    private $add;
     ...
     */
    protected $_pairsModeValue = 'list_id';
    
    /**
     * data base fields for assoc select
     */
    protected $_assocFields = array('id', 'list_id');
...
//        return $result;
    ...
    public function getList()
    {
        ...
        if ( $this->getFetchMode() == Warecorp_ICal_List_Enum_FetchMode::OBJECT ) {
            $query->from('calendar_event_lists', array('list_id'));
            $query->where('event_id = ?', $this->getEvent()->getId());
            $result = $this->DbConn->fetchCol($query);
            if ( sizeof($result) != 0 ) {
                foreach ( $result as &$item ) $item = new Warecorp_List_Item($item);
            }
            ...
            */
            ...
            $query->from('calendar_event_lists', array($this->getPairsModeKey(), $this->getPairsModeValue()));
            $query->where('event_id = ?', $this->getEvent()->getId());
    ...
    * 
    */
    public function add(Warecorp_List_Item $objList)
    {
        $this->add[] = $objList;
    }
    ...
    */
    public function deleteEventsAll()
    {
        ...
        $this->DbConn->delete('calendar_event_lists', $where);
    }

...
class BaseWarecorp_DDPages
{
    ...
    public static function resetBlocksStyles($entity){
        ...
        $result = Warecorp_DDPages::loadFromDB($entity->getId(), $entity->EntityTypeId);
        ...
        if (! empty($data)) {
           ...
           Warecorp_DDPages::saveToDB($entity->getId(), $entity->EntityTypeId, $data);
        }
    }
    /**
                ...
                if ($viewer) {
                    foreach ($data as $item) {
                        switch ($item["ContentType"]) {
                            ...
                            case 'ddGroupPhotos':
                                if ($access['photos']) {
                                    Warecorp_DDPages::getBlockHTML($_page, $content_array, $item, $entity);
                                }
                                break;
                            case 'ddMyLists':
                            case 'ddGroupLists':
    ...
    public static function getBlockHTML (&$_page, &$content_array, $item, $entity)
    {
        ...
        $content_item['template_type'] = ($content_item['target'] == 1) ? 'narrow' : 'wide';
        switch ($item["ContentType"]) {
             //-------------------------------------------------------------------------------------
            case 'ddMogulus':
                 if (! isset($item['Data'])) {
...
*/
    ...
    if (isset($item['Data'])) {
            ...
            case 'ddElectedOfficial':
                ...
                break;
            case 'ddDistrict':
                $regionalFlag = $entity->getContextRegionalFlag();
                $state = null;
                $district = null;

                if ( $regionalFlag !== null && $regionalFlag != '')
                {
                    $fname = substr($regionalFlag, 15);
                    $districtImage = BASE_URL.'/images/districts/'.$fname.'.gif';
                
                    $_page->Template->assign('districtImage', $districtImage);  
                    $_page->Template->assign('headline', 'District Map');  
                                    
                    $smarty_vars['Content'] = $_page->Template->getContents('content_objects/ddDistrict/preview_mode_' .  $content_item['template_type'] . '.tpl');
                }
                break;
            case 'ddGroupMap':
                ...
                if (isset($item['Data'])) {
                  ...
                  //  $children = array($entity);
                    ...
                    */
                    ...
                    // $entity = Warecorp_Group_Factory::loadById(132);
                    //$s['district'] = Z1SKY_Location_District::getDistrictsByStateCode(Warecorp_Location_City::create($entity->getCityId())->getState()->code);
                    ////////$s['city'] = $entity->getCityId();
                    //$s['district'] = 18;
                    ...
                    foreach ($groups as &$group) {
                       $group = Warecorp_Group_Factory::loadById($group,'simple');
                    }
            ...
            case 'ddGroupMembers':
                ...
                if ($item['Data']['default_index_sort'] == 1) {
                    ...
                    if (Warecorp::checkHttpContext(array('z1sky','zea','z350','zftn','zcon'))) {
                        foreach ($members as $id => $m) {
                            if (!Z1SKY_Group_AccessManager::cohostDeletable($group,$m)) {
                                unset($members[$id]);
                            }
                        }
                    }
                    ...
                    if (Warecorp::checkHttpContext(array('z1sky','zea','z350','zftn','zcon'))) {
                        $gml=$group->getMembers()->setListSize($item['Data']['display_number_in_each_region'])->setCurrentPage(1)->setOrder('zgm.status, zgm.creation_date DESC');
                        $members = $gml->getList();
                        ...
                        $_page->Template->assign('notDeletable', $notDeletable);
            ...
            case 'ddFamilyPeople':
                ...
                $smarty_vars['Content'] = $_page->Template->getContents('content_objects/ddFamilyPeople/preview_mode_' . $content_item['template_type'] .$groupMode. '.tpl');
            ...
            case 'ddGroupFamilyIcons':
                ...
                if (Warecorp_Group_BrandPhoto_Item::isPhotoExists($avatar_id)) {
                    ...
                    if ( ($fGroup->getMembers()->isMemberExistsAndApproved($_page->_user->getId()) && in_array($fGroup->getId(),  $group->getFamilyGroups()->returnAsAssoc(true)->setAssocValue('id')->getList()) )    || ($group->getMembers()->isMemberExistsAndApproved($_page->_user->getId()) && in_array($currentBrandPhoto->getGroupId(),  $group->getFamilyGroups()->returnAsAssoc(true)->setAssocValue('id')->getList())  )    || $avatar_id = 0) {
                        $_page->Template->assign('currentAvatar', $currentBrandPhoto);
                    } else {
                        $_page->Template->assign('currentAvatar', new Warecorp_Group_BrandPhoto_Item() );
                    }
            ...
            case 'ddFamilyDiscussions':
                ...
                if ($group->getGroupType() == 'family') {
                    ...
                    $groupsList = $group->getGroups()->setTypes(array(
                        'simple'))->getList();
                ...
                if (! empty($item['Data']['discussionsShowThreadsNumber']) && (! empty($item['Data']['discussionsDisplayMostActive']) || ! empty($item['Data']['discussionsDisplayMostRecent']))) {
                    ...
                    if ($group->getGroupType() == 'family') {
                        //$fGroupsList = array();
                        foreach ($group->getGroups()->returnAsAssoc(true)->setAssocValue('id')->getList() as $_v) {
                            $fGroupsList[] = $_v;
                        }
                    }
                    $fGroupsList = array_unique($fGroupsList);
            ...
            case 'ddMyDiscussions':
                ...
                $discussionList = new Warecorp_DiscussionServer_DiscussionList();
                $groupsList = $userInfo->getGroups()->/*setTypes(array(
                    'family' ,
                    'simple'))->*/getList();
                foreach ($groupsList as &$_v) {
                    $tmpList = $discussionList->findByGroupId($_v->getId());
                    foreach ($tmpList as &$_vv) {
                        $_group = Warecorp_Group_Factory::loadById($_vv->getGroupId());
            ...
            case 'ddGroupPhotos':
                ...
                if ($item['Data']['gallery_type'] == 1) {
                    ...
                    if (Warecorp_Photo_AccessManager::canViewPrivateGalleries($group, $_page->_user)){
                        ...
                        $galleries = $group->getGalleries()->setPrivacy(0)->setSharingMode('own')->getList();
                    }
            ...
            case 'ddMyGroups':
                ...
                if (isset($item['Data'])) {
                    $_page->Template->assign($item['Data']);
                    $_page->Template->assign('family_groups_empty', (empty($item['Data']['family_unhide']) || count($item['Data']['family_unhide']) == 0));
                    $_page->Template->assign('groups_empty', (empty($item['Data']['unhide']) || count($item['Data']['unhide']) == 0));
                }
            ...
            case 'ddRSSFeed':
             ...
             if (false && strpos($item['Data']['rss_url'],BASE_HTTP_HOST)>0){
                $_smarty->assign('LOCALE',LOCALE);
                $_smarty->assign('isInternalFeed',1);
             }else{

                //require_once (MAGPIE_DIR . 'rss_fetch.inc');
                //require_once (MAGPIE_DIR . 'rss_utils.inc');

                $item['Data']['rss_url'] = str_replace('feed://', 'http://', $item['Data']['rss_url']);
                $item['Data']['rss_url'] = str_replace('feed:http', 'http', $item['Data']['rss_url']);
                $smarty_vars["title"] = empty($item['Data']['rss_title']) ? '' : $item['Data']['rss_title'];




                //NEW SCHOOL (ZEND_FEED)

                try {$slashdotRss = Zend_Feed::import($item['Data']['rss_url']);
                } catch (Exception $e) {
                    $slashdotRss = array();
                }

                if (is_object($slashdotRss)) {

                    $smarty_vars["title"] = empty($item['Data']['rss_title']) ? $slashdotRss->title() : $item['Data']['rss_title'];

                    $rss_hash = array();
                    $_cntr = 0;
                    foreach ($slashdotRss as $rssitem) {

                        $_cntr++;
                        if ($_cntr>intval($item['Data']['rss_max_lines'])) break;
                        $record["href"] = Warecorp_Common_Functions::getFirstFeedNode($rssitem->link());
                        $record["title"] = Warecorp_Common_Functions::getFirstFeedNode($rssitem->title());
                        $record["description"] = '';
                        $_description = Warecorp_Common_Functions::getFirstFeedNode($rssitem->description());
                        $_atom_content = Warecorp_Common_Functions::getFirstFeedNode($rssitem->atom_content());

                        if (! empty($_description))
                            $record["description"] = empty($item['Data']['rss_view']) ? '' : Warecorp_Common_Functions::getFirstFeedNode($rssitem->description()); elseif (! empty($_atom_content))
                            $record["description"] = empty($item['Data']['rss_view']) ? '' : Warecorp_Common_Functions::getFirstFeedNode($rssitem->description());
                        if ($item['Data']['rss_view'] == 2 /*|| $content_item['template_type'] == 'narrow'*/)
                        {
                        ...
                        if (is_string($record["description"])) {
                            ...
                            $record["description"] = preg_replace_callback($pattern, 'Warecorp_DDPages::prcw', $record["description"]);
                            $pattern = "/(\<object)([^\>]*)(height=\")([0-9]*)(\")([^\>]*)(\>)/i";
                            $record["description"] = preg_replace_callback($pattern, 'Warecorp_DDPages::prch', $record["description"]);
                        }
                ...
                //OLD SCHOOL (MAGPIE)
                //$rss = fetch_rss($item['Data']['rss_url']);

                if (false && is_object($rss)) {
                    $smarty_vars["title"] = empty($item['Data']['rss_title']) ? $rss->channel['title'] : $item['Data']['rss_title'];
                    $rss_hash = array();
                    for ($i = 0; $i < $item['Data']['rss_max_lines']; $i ++) {
                        if (! isset($rss->items[$i]))
                            break;
                        $record["href"] = $rss->items[$i]['link'];
                        $record["title"] = $rss->items[$i]['title'];
                        $record["description"] = '';
                        if (! empty($rss->items[$i]['description']))
                            $record["description"] = empty($item['Data']['rss_view']) ? '' : $rss->items[$i]['description']; elseif (! empty($rss->items[$i]['atom_content']))
                            $record["description"] = empty($item['Data']['rss_view']) ? '' : $rss->items[$i]['atom_content'];
                        if ($item['Data']['rss_view'] == 2 /*|| $content_item['template_type'] == 'narrow'*/)
                        {
                            $record["description"] = urldecode($record["description"]);
                            $pattern = "/\<img[^\>]*\>/i";
                ...
                $smarty_vars['Content'] = $_smarty->getContents('content_objects/ddRSSFeed/preview_mode_' . $content_item['template_type'] . '.tpl');
                break;
            ...
            /*case 'ddProfileHeadline' :
                ...
                $smarty_vars["Content"] = $headline;
                $_page->Template->assign($smarty_vars);
                $smarty_vars["Content"] = $_page->Template->getContents('content_objects/ddProfileHeadline/preview_mode.tpl');
            ...
            /*case 'ddProfileIntroduction' :
                ...
                $smarty_vars["Content"] = $intro;
                $_page->Template->assign($smarty_vars);
                $smarty_vars["Content"] = $_page->Template->getContents('content_objects/ddProfileIntroduction/preview_mode.tpl');
            ...
            case 'ddGroupDescription':
                ...
                if (! isset($item['Data'])) {
                    $item['Data'] = array();
                    $item["Data"]["font_family"] = '';
                    $item["Data"]["font_size"] = '';
                    $item["Data"]["font_weight_bold"] = 0;
                    $item["Data"]["font_style_italic"] = 0;
                    $item["Data"]["text_decoration_underline"] = 0;
                    $item["Data"]["text_align"] = '';
                    $item["Data"]["color"] = '';
                }
                $item['Data']['Content'] = $description;

                $_page->Template->assign($item['Data']);
                $_page->Template->assign('cloneId', $item['ID']);

                $smarty_vars["Content"] = $_page->Template->getContents('content_objects/ddGroupDescription/preview_mode_' . $content_item['template_type'] . '.tpl');
                break;
            // + -------------------------------------------------------------------------------------
            case 'ddGroupHeadline':
                ...
                $smarty_vars["Content"] = $_page->Template->getContents('content_objects/ddGroupHeadline/preview_mode_' . $content_item['template_type'] . '.tpl');
                break;
            //-------------------------------------------------------------------------------------
            case 'ddGroupImage':
                $groupInfo = Warecorp_Group_Factory::loadById($entity->getId());
        ...
        if (sizeof($arr) != 0) {
            foreach ($arr as $item_name => $item_value) {
                if (is_array($item_value)) {
                    ...
                    Warecorp_DDPages::phpArr2jsArr($Code, "" . $Var . "[\"" . $item_name . "\"]", $item_value);

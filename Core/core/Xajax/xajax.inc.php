 ...
 *
 * @package xajax
 * @version $Id: xajax.inc.php,v 1.7 2007/03/05 13:20:12 sukharev Exp $
...
class xajax
{
	...
	/**
	 * @var array Array of RequestURI to be used with each function (key=function name)
	 */
	var $aFunctionRequestURIs;
	/**
	...
	/**
	 * @param mixed  contains the function name or an object callback array
	 * @param mixed  contains the URI
	 * @param mixed  request type (XAJAX_GET/XAJAX_POST) that should be used
	 *               for this function.  Defaults to XAJAX_POST.
	 */
	function registerUriFunction($mFunction, $sRequestURI, $sRequestType=XAJAX_POST)
	{
        $request = Zend_Controller_Front::getInstance()->getRequest();
        $controller = $request->getControllerName();

		if (is_array($mFunction)) {
		    $params = explode('/', $sRequestURI);

            /** added functionality according routes in ZF 1.10 **/
            if ( $params[1] === 'groups' && $controller === 'groups' ) {
                if ( $request->has('groupname') ) {
                    $params[2] = 'group/'.$request->getParam('groupname').'/'.$params[2];
                }
			    $sRequestURI = ( HTTPS_ENABLED && isset($_SERVER['HTTPS']) ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[2].'/xajaxcontext/'.$params[1];
            } else if ( $params[1] === 'users' && $controller === 'users' ) {
                if ( $request->has('username') ) {
                    $params[2] = 'user/'.$request->getParam('username').'/'.$params[2];
                }
			    $sRequestURI = ( HTTPS_ENABLED && isset($_SERVER['HTTPS']) ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[2].'/xajaxcontext/'.$params[1];
            } else {
                if ( $_SERVER['HTTP_HOST'] != BASE_HTTP_HOST ) {
                    ...
                    $sRequestURI = ( HTTPS_ENABLED && isset($_SERVER['HTTPS']) ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[1].'/'.$params[2].'/xajaxcontext/'.$params[1];
                }
            }
            /****/
		    ...
		    $this->aFunctions[$mFunction[0]] = 1;
			$this->aFunctionRequestTypes[$mFunction[0]] = $sRequestType;
			$this->aFunctionRequestURIs[$mFunction[0]] = $sRequestURI;
			$this->aObjects[$mFunction[0]] = array_slice($mFunction, 1);
		}
		else {
            $params = explode('/', $sRequestURI);
            ...
            if ( $params[1] === 'groups' && $controller === 'groups' ) {
                ...
                /**
                 * for EIA group is Global Group
                 * @author Artem Sukharev 
                 */
                elseif ( Zend_Registry::isRegistered('globalGroup') ) {
                    $currentGroup = Zend_Registry::get('globalGroup');
                    $params[2] = 'group/'.$currentGroup->getPath().'/'.$params[2];
                }
			    $sRequestURI = ( HTTPS_ENABLED && isset($_SERVER['HTTPS']) ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[2].'/xajaxcontext/'.$params[1];
            } else if ( $params[1] === 'users' && $controller === 'users' ) {
                if ( $request->has('username') ) {
            ...
            while(!empty($params[$i])) {
                if (!empty($params[$i + 1]))
                    $sRequestURI.="/".$params[$i]."/".$params[$i + 1];    
                $i = $i + 2;
            }            
			$this->aFunctions[$mFunction] = 1;
			$this->aFunctionRequestTypes[$mFunction] = $sRequestType;
			$this->aFunctionRequestURIs[$mFunction] = $sRequestURI;
		}
	}
	/**
	...
	function getRequestParams()
	{
		...
		if ($requestMode == XAJAX_POST) {
			if ( isset($_POST["xajaxargs"]) ) {
				$aArgs = $_POST["xajaxargs"];
			}
		} else {
			if ( isset($_GET["xajaxargs"]) ) {
	...
	function processRequests()
	{
		...
		// Определяем название функции, которую надо выполнить
		...
		// Переопределение обрабтчика обишок если надо
		//____________________________________________________
		if ($this->bErrorHandler) {
			$GLOBALS['xajaxErrorHandlerText'] = "";
			set_error_handler("xajaxErrorHandler");
		}
        // Проверяем, установлена ли функция, которую надо выполнить перед
        // процессингом, если да, то проверяем, можем ли ее выполнить
        //____________________________________________________
		...
		// Если фукция, которую надо выполнить, требует подключения файла,
		// подключаем его
		//____________________________________________________
		...
		if ($bFoundFunction) {
			...
			// если в массиве зарегестрированных фукнций
			//____________________________________________________
		...
		if ($bFoundFunction)
		{
            ...
            // Если установлена функция, которую надо выполнить перед
            // процессингом, выполняем ее
            //____________________________________________________
			if ($this->sPreFunction) {
            ...
            // если ранее реквест не был закрыт, то выполняем основную фукнцию
            //____________________________________________________
			if (!$bEndRequest) {
	...
	function getJavascriptConfig()
	{
		...
		$html .= "var xajaxClearURI=true;\n";

		foreach($this->aFunctions as $sFunction => $bExists) {
		    if ( isset($this->aFunctionRequestURIs[$sFunction]) ) {
		        $html .= $this->_wrapURI($sFunction, $this->aFunctionRequestURIs[$sFunction], $this->aFunctionRequestTypes[$sFunction]);
		    } else {
	...
	function getJavascriptInclude($sJsURI="", $sJsFile=NULL)
	{
		...
		$html .= "\t<script type=\"text/javascript\" src=\"".JS_URL."/xajaxRequestManager.js\"></script>\n";
		$html .= "\t<script type=\"text/javascript\">\n";
	...
	function _wrapURI($sFunction, $sRequestURI, $sRequestType=XAJAX_POST)
	{
		$js = "function ".$this->sWrapperPrefix."$sFunction(){return xajax.call_uri(\"$sRequestURI\", arguments, ".$sRequestType.");}\n";
		return $js;
	}
	/**

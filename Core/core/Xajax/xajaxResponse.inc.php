 ...
 *
 * @package xajax
 * @version $Id: xajaxResponse.inc.php,v 1.2 2007/02/20 08:04:37 sukharev Exp $
...
class xajaxResponse
{
	...
	/**
	 * show zanby popup
	 * @param string $sData - content for innerHTML
	 * @param object $sAttribute
	 * @author Artem Sukharev
	 */
	function addProcessMessage($sData, $sAttribute = null)
	{
	    $this->xml .= $this->_cmdXML(array("n" => "pm", "p" => $sAttribute),$sData);
	}
	
    /**
     * Show Ajax Message ( MainApplication.showAjaxAlert(); )
     * @param string $sData - content for innerHTML
     * @param object $sAttribute
     * @author Artem Sukhare
     */	
    function showAjaxAlert($sData, $sAttribute = null)
    {
        $this->xml .= $this->_cmdXML(array("n" => "aa", "p" => $sAttribute),$sData);
    }
    
    function addDownloadFile($sUrl)
    {
        $this->xml .= $this->_cmdXML(array("n" => "df", "u" => $sUrl),'');
    }
...
//			$xml .= " $sAttribute=\"$sValue\"";
		...
		foreach($aAttributes as $sAttribute => $sValue) {
			if ( $sAttribute ) {
			    if ( is_object($sValue) ) {
                    $xml .= " $sAttribute='".Zend_Json::encode($sValue)."'";
     ...
     * print xml
     */
	function printXml($sEncoding="")
	{
	    $response = "";
        $sContentHeader = "Content-type: text/xml;";
        if ($sEncoding && strlen(trim($sEncoding)) > 0) $sContentHeader .= " charset=".$sEncoding;
        if (is_a($this, "xajaxResponse")) $response = $this->getXML();

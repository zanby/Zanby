...
class bbcode {
	...
	var $mnemonics = array(
		':)' => '<img src="/img/discussion/smilies/zanby/smail.gif">',
		':-)))' => '<img src="/img/discussion/smilies/zanby/laugh.gif">',
		':-D' => '<img src="/img/discussion/smilies/zanby/laugh.gif">',
		':-(' => '<img src="/img/discussion/smilies/zanby/sadness.gif">',  
	);
	/**
	*  :-)    улыбающийся
	*  :-)))   смеющийся
	...
	* [:|||:]   баян
	* [:]    робот
	* ГгY или \o/ или ("\(о_О)/")   превед, медвед
	* 
	* АЗИАТСКИЕ СМАЙЛИКИ
...
class bb_code extends bbcode {
	...
	var $children = array();
	function get_html() {
		$str = '<div class="bb" style="overflow:auto; font-size:11px; border:1px solid #CFCFCF; width:97%; max-height:250px; padding:5px;">';
		foreach ($this -> tree as $item) {
			if ('item' == $item['type']) { continue; }
...
class bb_php extends bbcode {
	...
	function get_html() {
		...
		return '<div class="php" style="overflow:auto; font-size:11px; border:1px solid #CFCFCF; width:97%; max-height:250px; padding:5px;">'.$str.'</div>';
	}
}
		...
		*/
		if ($author = htmlspecialchars($this -> attrib['quote'])) {
			...
			if ( $post->getId() !== null ) {
				...
				$return =
					Warecorp_DiscussionServer_Post::formatQuoteStart().
					parent::get_html($this -> tree).
					Warecorp_DiscussionServer_Post::formatQuoteEnd();
			}
		} else {
			$return =
				Warecorp_DiscussionServer_Post::formatQuoteStart().

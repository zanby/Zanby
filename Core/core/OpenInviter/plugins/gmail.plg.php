...
class gmail extends openinviter_base
	...
	public function getMyContacts()
		...
		// Trim result to only the XML body
        $pos = strpos($res, '<');
        $xml = substr($res, $pos);

        //Create XML object and register necessary namespaces
        $feed = new SimpleXMLElement($xml);
        $feed->registerXPathNamespace('default', 'http://www.w3.org/2005/Atom');
        $feed->registerXPathNamespace('gd', 'http://schemas.google.com/g/2005');

        foreach ((array)$feed->xpath('//default:entry') as $entry) {
            
            $name = (string)$entry->title;
            
            foreach ((array)$entry->xpath('gd:email') as $email) {
                $attributes = $email->attributes();
                $address = (string)$attributes->address;
                $contacts[$address] = array('first_name' => $name, 'email_3' => $address); 
            }
        }
